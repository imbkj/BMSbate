...
<%
	...
	poCtrl.addCustomToolButton("保存", "Send", 3);
	...
	String sqlstr = "select cid,gid,eada_name name,eada_fileplace,(select case when Substring(convert(varchar(10),MAX(eada_sdh)),1,4)=YEAR(GETDATE()) then MAX(eada_sdh)+1 else convert(varchar(10),YEAR(GETDATE()))+ '001' end eada_sdh from EmArchiveDatum where eada_state=1)"
			+ " ,eada_tapr_id,eada_sdh,eada_idcard idcard from EmArchiveDatum where eada_state=1 and  eada_id="
			+ eada_id;
	...
	while (rs.next()) {
		...
		DataTag idcard = doc.openDataTag("{身份证}");
		idcard.setValue("" + rs.getString("idcard"));
...
<body>
	<script type="text/javascript">
		...
		function Send() {
			...
			window.location.href="../Archives/Archive_SendOk.jsp?taprid=<%=taprId%>&eadaid=<%=eada_id%>&sdh=<%=sdh%>&userid=<%=userid%>&mp=<%=mp%>&taclId="+window.location.search.split("=")[1];
		}
	</script>
	<form id="form1">
