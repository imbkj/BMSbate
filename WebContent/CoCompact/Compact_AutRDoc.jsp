...
<%
	...
	poCtrl.addCustomToolButton("打印", "Save", 1);
	poCtrl.addCustomToolButton("通过", "Send", 2);
	//设置保存页面
	...
	String sql2 = "select count(*),puof_url from PubOffice where puof_pute_id=2 and puof_tid="
			+ request.getParameter("coco_id") + " group by puof_url,puof_id order by puof_id";
...
<body>
	<script type="text/javascript">
		...
		function Send() {
			...
			window.location = "../CoCompact/Compact_AutOk.jsp?coco_id=<%=request.getParameter("coco_id")%>&coco_tapr_id=<%=request.getParameter("coco_tapr_id")%>&user=<%=request.getParameter("user")%>&coco_class=<%=request.getParameter("emco_class")%>&aut_st=1&way=<%=request.getParameter("way")%>";
		}
