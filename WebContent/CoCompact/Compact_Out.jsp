...
<body>
	<%
	int way= Integer.parseInt(request
			.getParameter("way"));
	%>
	<table>
		<tr>
		...
		<tr>
			<td><button
					onclick="window.location = '../CoCompact/Compact_Out.jsp?coco_id=<%=request.getParameter("coco_id")%>&coco_tapr_id=<%=request.getParameter("coco_tapr_id")%>&user=<%=request.getParameter("user")%>&sub=1&way=<%=request.getParameter("way")%>&remark='+out_remark.value ">提交</button></td>
		</tr>
	</table>
	<%
		...
		if (request.getParameter("sub").equals("1")) {
			...
			Object[] obj = {"7", coco_id, request.getParameter("remark")};
			//执行工作流

			WfOperateService wf = new WfOperateImpl(
					new CoCompact_OperateImpl());
			String[] m = wf.ReturnToN(obj, coco_tapr_id, 2,user,request.getParameter("remark"));
