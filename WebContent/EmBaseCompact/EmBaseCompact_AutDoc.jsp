...
<%
	...
	//添加自定义按钮
	poCtrl.addCustomToolButton("暂存", "Save", 0);
	...
	if (cont > 0) {
		poCtrl.webOpen("../OfficeFile/DownLoad/EmBaseCompact/" + ofile,
				OpenModeType.docRevisionOnly, "szciic");
	} else {
		poCtrl.webOpen(
...
<body>
	<script type="text/javascript">
		...
		function Send() {
			document.getElementById("PageOfficeCtrl1").WebSave();
			window.location = "../EmBaseCompact/EmBaseCompact_AutDocOk.jsp?ebcc_id=<%=request.getParameter("ebcc_id")%>&user=<%=request.getParameter("user")%>&ebcc_tapr_id=<%=request.getParameter("ebcc_tapr_id")%>	";
		}
	</script>
	<form id="form1">
