...
<% 
	int ebcu_tapr_id = Integer.parseInt(request.getParameter("ebcu_tapr_id"));
	int ebcc_id = Integer.parseInt(request.getParameter("ebcc_id"));
	int userid = Integer.parseInt(request.getParameter("user"));
	LoginDal dal = new LoginDal();
	String user = dal.getUsernameById(userid);
	 //链接数据库操作开始
	 //String sql = "update EmBaseCompactUpdate set ebcu_upstate=1 where ebcu_id="+ebcc_id;
...
<body>
	<script type="text/javascript">
		alert('审核成功！');
		//window.location.href = '../EmBaseCompact/EmBaseCompactSA_AutList.zul';
	</script>
</body>
</html>