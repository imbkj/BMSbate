...
public class wtoutFeeBll  implements WfBusinessService  {
	...
	// 获取城市列表
		...
		public List<CoAgencyBaseCityRelViewModel> getDefaultCoAgencylist(Integer id) {
			return new Standard_ListDal().getDefaultCoAgencylist(id);
		}
		...
		public String[] Wtfeeadd(wtoutFeeModel m,String company)
		{
			...
			Object[] obj = {"1", m};
			WfOperateService wfs = new WfOperateImpl(new wtoutFeeBll());
		...
		//禁止使用服务费
				...
				public String[] Wtfeestop(wtoutFeeModel m)
				{
					String[] str = new String[2];
					EmCommissionOutListBll bll = new EmCommissionOutListBll();
					//去掉禁止的限制
					/*
					if (bll.getEmCommOutList(" AND a.ecou_addtype NOT IN ('离职','退回')  AND d.Wtot_feeid= "+m.getWtot_feeid() ).size()>0)
					{
						str[0]="0";
						str[1]="此服务费尚有员工，请处理后在操作禁止！";
					...
					*/
					 ...
					 if (m.getWtot_state()==0)
					{
						...
						str[0]="0";
						str[1]="此服务费尚未通过审核，不能禁止！";
					}
					else
						
					{
						wtoutFeeDal dal=new wtoutFeeDal();
						m.setWtot_state(5);//禁止状态修改
						m.setWtot_ifview(0);
						
						m.setWtot_editname(UserInfo.getUsername());
						m.setWtot_comfdate(DateStringChange.StringtoDate(DateStringChange.Datestringnow("yyyy-MM-dd"),"yyyy-MM-dd"));
						if (dal.Wtfeechangeupdate(m)>0)
						{
							str[0]="1";
							str[1]="服务费禁止成功!";
						}
		...
		public int wtfeeupdateownmonth(wtoutFeeModel m)
		{
			int i=0;
			wtoutFeeDal dal = new   wtoutFeeDal();
			i=dal.Wtfeeupdate(m);
...
//			
		...
		public Boolean UpdateTaprid(int dataId, int tapr_id, int state) {
			return new wtoutFeeDal().UpdateTaprid(dataId, tapr_id);
		}
