...
public class CoQuotation_editywyzxController {
	...
	// 初始化
	public CoQuotation_editywyzxController() throws Exception {
		...
		try {
			...
			try {
				...
				coof_id = Integer.parseInt(Executions.getCurrent().getArg()
						.get("colaid").toString());
				//读报价单？？
				coofList = bll.getcoofListcoofid(coof_id, null);
				if(coofList.size()>0)
				{
					cfModel=coofList.get(0);
					coofModel=cfModel;
				}
//				System.out.print(cfModel.getCoof_name1());
				if (cfModel.getCoof_name1()!=null) {
					coofNameChange1(cfModel.getCoof_name1());
				}
...
//				
	...
	@NotifyChange({ "cfModel", "is_tjbj", "vis_rsgm", "cpctModel",
			"notselectList", "agencyList" })
	public void coofNameChange1(@BindingParam("coofname") String coofname) {
		CoQuotation_AddBll bll = new CoQuotation_AddBll();
		try {
			if (coofModel != null) {
