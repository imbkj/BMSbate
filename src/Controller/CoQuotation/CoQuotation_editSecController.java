...
public class CoQuotation_editSecController {
	...
	// 初始化
	public CoQuotation_editSecController() throws Exception {
	...
	@NotifyChange({ "clModel", "fwvis", "flvis", "fwinvis", "standardList",
			...
			"int57", "int58", "xstjvis", "xstjvis1", "xstjvis2", "flpkvis",
			"rspkvis", "hjpkvis", "feeReadonly", "feetype" })
	...
	public void span(@BindingParam("gd") Grid gd) {
		...
		if (ckbcount > 1) {
			...
			// for (int i = 0; i < infoList.size() - 1; i++) {
			// for (int j = 0; j < infoList.size() - 1 - i; j++) {
			// if (infoList.get(j).getColi_group_count() == 1
			// && infoList.get(j).getColi_group_count() != infoList
			// .get(j + 1).getColi_group_count()) {
			// CoOfferListModel m = infoList.get(j);
			// infoList.set(j, infoList.get(j + 1));
			// infoList.set(j + 1, m);
			// }
	...
	public void selectchange() {
		if (clModel.getColi_isfwf() != null && clModel.getColi_isfwf() == 1) {
			...
			if (clModel.getCopr_type().equals("服务产品")
					...
					&& (!clModel.getColi_name().equals("服档费")
							&& !clModel.getColi_name().equals("综合管理费") && !clModel
	...
	public void initfl() {
		try {
			...
			System.out.println(clModel.getColi_name() + ":"
					+ standardList.size());
		...
		try {
			...
			if (su == 1) {
				...
				Window window = (Window) Executions.createComponents(
						"/CoQuotation/CoQuotation_edit.zul", null, map);
				window.doModal();
			}
		} catch (Exception e) {
						...
						if (Messagebox.Button.OK.equals(event.getButton())) {
							...
							if (su == 1) {
								...
								if (Messagebox.show("提交成功!", "INFORMATION",
									...
									win.detach();
									if (UserInfo.getDepID().equals("7")) {
										Executions
												.sendRedirect("/CoQuotation/CoQuotation_OperateList.zul");
									}
