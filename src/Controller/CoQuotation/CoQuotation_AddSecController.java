...
public class CoQuotation_AddSecController {
	...
	// 初始化
	public CoQuotation_AddSecController() throws Exception {
		// 报价单ID
		coli_coof_id = Integer.parseInt(Executions.getCurrent().getArg()
				.get("coofid").toString());
		System.out.println(coli_coof_id);
	...
	@NotifyChange({ "clModel", "fwvis", "flvis", "fwinvis", "standardList",
			...
			"int57", "int58", "xstjvis", "xstjvis1", "xstjvis2", "feeReadonly",
			"flpkvis", "rspkvis", "hjpkvis", "feetype" })
	...
	@NotifyChange({ "fwvis", "flvis", "fwinvis", "standardList",
			...
			"xstjvis2", "flpkvis", "rspkvis", "hjpkvis", "feetype" })
		...
		if (clModel.getColi_isfwf() != null && clModel.getColi_isfwf() == 1) {
			...
			if (clModel.getCopr_type().equals("服务产品")
					&& (clModel.getColi_name() != null
							...
							.getColi_name().equals("综合管理费") && !clModel.getColi_name().equals("保管服务费"))) {
				setFwinvis(true);
				setFwvis(false);
				setFlvis(false);
				setFlpkvis(false);
