...
public class CoQuotation_ItemAddqgController {
	...
	// 初始化数据
	public CoQuotation_ItemAddqgController() {
		...
		/*
		 * try { search(); } catch (Exception e) { // TODO Auto-generated catch
	...
	public void search() throws Exception {
		try {
			Combobox cbcoproduct = (Combobox) win.getFellow("coproduct");
			cbcoproduct.setValue("");
			str = "";
			CoQuotation_AddBll bll = new CoQuotation_AddBll();
			...
			if (clModel.getCopr_type().equals("服务产品")
				...
				if (clModel.getColi_pclass().equals("员工福利")) {
					...
					if (standardList.size() > 1) {
						clModel.setColi_standard(standardList.get(1)
								.getColi_standard());
					}
	...
	@Command
	public void submit() {
		Combobox cb = (Combobox) win.getFellow("coproduct");
