...
import bll.CoCompact.Compact_AddSelectBll;

public class Compact_AddSelectController {
	private List cocoBaseList; // 合同基本信息列表
	private List cocoAddnameList; // 添加人列表
	private Compact_AddSelectBll cocoBll = new Compact_AddSelectBll();
	...
	public Compact_AddSelectController() {
		// 首页加载显示所有数据
	...
	public void compact_add(@BindingParam("coabM") CoCompactModel coabM) {
		...
		Window wnd = (Window) Executions.createComponents("Compact_Doc.zul", null, arg);  
		wnd.doModal();  
	}
	...
	//制作补充协议
		...
		public void compact_bcadd(@BindingParam("coabM") CoCompactModel coabM) {
			...
			arg.put("id", coabM.getCoco_tapr_id());
			arg.put("daid", coabM.getCoco_id());
			Window wnd = (Window) Executions.createComponents("Compact_BcUpload.zul", null, arg);  
			wnd.doModal();  
		}
	
	//上传纸质合同
		@Command("compact_upload")
		public void compact_upload(@BindingParam("coabM") CoCompactModel coabM) {
			
			Map arg = new HashMap();  
			arg.put("id", coabM.getCoco_tapr_id());
			arg.put("daid", coabM.getCoco_id());
			Window wnd = (Window) Executions.createComponents("Compact_Upload.zul", null, arg);  
			wnd.doModal();  
		}
