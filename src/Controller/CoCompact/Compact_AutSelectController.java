...
import bll.CoCompact.Compact_AutSelectBll;

public class Compact_AutSelectController {
	private List cocoBaseList; // 合同基本信息列表
	private List cocoAddnameList; // 添加人列表
	private Compact_AutSelectBll cocoBll = new Compact_AutSelectBll();
	...
	public Compact_AutSelectController() {
		// 首页加载显示所有数据
	...
	public void compact_add(@BindingParam("coabM") CoCompactModel coabM) {
		...
		arg.put("puf_id", 0);
		arg.put("sc", 2);
		Window wnd = (Window) Executions.createComponents("Compact_AutDoc.zul", null, arg);  
		wnd.doModal();  
	}
