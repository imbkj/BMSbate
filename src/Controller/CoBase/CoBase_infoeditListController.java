...
public class CoBase_infoeditListController{
	private String str = "";
	private String strs = "";
	...
	public CoBase_infoeditListController() {
		...
		if (!pubtreelist.isEmpty()) {
			...
			for (int y = 0; y < pubtreelist.size(); y++) {
				...
				if (model.getPid() == 0) {
					...
					// 调用递归函数
					AddTreeItem(newitem, model.getId(), pubtreelist, tree);
				}
			}
		}
	}
	...
	private void AddTreeItem(Treeitem ptem, int parentID,
			List<pubTreeModel> list, Tree tree) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (model.getPid() == parentID) {
				...
				tw.setParent(item);
				// 函数递归
				AddTreeItem(item, model.getId(), list, tree);
			}
		}
	}
