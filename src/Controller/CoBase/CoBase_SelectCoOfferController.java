...
public class CoBase_SelectCoOfferController {
	...
	public CoBase_SelectCoOfferController() {
		try {
			if (Executions.getCurrent().getArg().get("cid") != null) {
				cid = Integer.valueOf(Executions.getCurrent().getArg()
						.get("cid").toString());
			}
			if (Executions.getCurrent().getArg().get("coco_compactid") != null) {
			...
			//
			// System.out.println(coco_compactid);
	...
	public void chakan(@BindingParam("model") CoOfferModel model) {
		...
		Window window = (Window) Executions.createComponents(
				"/CoQuotation/CoQuotationInfoChange.zul", null, map);
		window.doModal();
	}
						...
						public void onEvent(ClickEvent event) throws Exception {
							...
							if (Messagebox.Button.OK.equals(event.getButton())) {
								if (sbll.delCooffer(m.getCoof_id())) {
									...
									Messagebox.show("删除失败,请联系IT部.", "操作提示", Messagebox.OK,
											Messagebox.ERROR);
								}
							}
						}
