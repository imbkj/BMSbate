...
public class EmCommissionOut_OneTimeFeeDetailController {
	...
	public EmCommissionOut_OneTimeFeeDetailController() {
		try {
	...
	public void init() {
		...
		try {
			setM(bll.getEmCommOutChangeInfo(daid, " and type='ecocotf'"));
			setFeeList(bll
					.getFeeDetailChangeList(" and eofc_month_sum>0 and eofc_ecoc_id="
							+ daid));
			setHosList(bll.getHosList(daid, " and pbsr_type='ecocotf'"));
