...
public class Standard_ModController {
	...
	private boolean feeVis1;
	private boolean feeVis2;

	public Standard_ModController() {
		try {
	 ...
	 * 
	 */
	@Command("ServiceFeeOnChange")
	@NotifyChange({ "feeVis1", "feeVis2" })
	public void ServiceFeeOnChange() {
		try {
			if (m.getEcos_service_fee().compareTo(m.getService_cost()) == -1) {
				feeVis1 = false;
				feeVis2 = true;
			} else {
				feeVis1 = true;
			...
			Messagebox.show("服务费收支比较出错!", "ERROR", Messagebox.OK,
					Messagebox.ERROR);
			e.printStackTrace();
		}
	}
		...
		try {
			if (caM.getCoab_name().equals("无")) {
				...
				if (m.getEcos_service_fee().compareTo(m.getService_cost()) == -1) {
					...
					new Standard_OperateBll().CoOfferListMod(m.getEcos_id(),
							set);
				...
				// 服务费支出大于等于收入
					...
					if (Messagebox.show("服务费支出大于等于收入，确定提交?", "CONFIRM",
							Messagebox.OK | Messagebox.CANCEL,
							Messagebox.QUESTION) == Messagebox.OK) {
						m.setEcos_state(m.getEcos_state() + 1);
						...
						String[] str = new Standard_OperateBll().UpdateState(m);
						new Standard_OperateBll().CoOfferListMod(
								m.getEcos_id(), set);
	...
	public boolean isFeeVis1() {
		return feeVis1;
	}

	public void setFeeVis1(boolean feeVis1) {
		this.feeVis1 = feeVis1;
	}

	public boolean isFeeVis2() {
		return feeVis2;
	}

	public void setFeeVis2(boolean feeVis2) {
		this.feeVis2 = feeVis2;
	}
