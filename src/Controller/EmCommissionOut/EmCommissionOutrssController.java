...
public class EmCommissionOutrssController extends SelectorComposer<Component> {
	...
	public EmCommissionOutrssController() throws Exception {
		...
		setEmcomm(bll.getemcommtlistall(1, cbcity, wtjgname));
		setEcycmodellist(bll.searchembaselisthd(wtztlist.indexOf(wtzt), cbcity,
				wtjgname));
		if (emcomm.size() > 0) {
	...
	public void selectlist(@BindingParam("grd") Grid grid) {
		try {
			...
			// wtztlist.add(0,"未采集");
			// wtztlist.add(1,"未审核");
			// System.out.println(wtztlist);

			System.out.println(cbcity);
			System.out.println(wtjgname);
			...
			if (cbcity.equals("")) {
				...
				setEmcomm(bll.getemcommtlistall(1, cbcity, wtjgname));
				setEcycmodellist(bll.searchembaselisthd(wtztlist.indexOf(wtzt),
						cbcity, wtjgname));
				setModel(emcomm.get(0));
				colsInit(grid);
			}
	...
	public void colsInit(@BindingParam("self") Grid gridcols) throws Exception {
		...
		if (model != null) {
			...
			cbone.setValue("选择");
			cbone.addEventListener("onCheck", new MyListenerCheckall(cbone,
					gridcols));
	...
	public void rowsInit(Grid gridrows) throws Exception {
		...
		for (int m = 0; m < ecycmodellist.size(); m++) {
			...
			if (model.getEcyt_ylao() == 1) {
				...
				lab5.setParent(gdr);
				lab5.setValue(df.format(ecycmodellist.get(m).getEcyc_sb_base())
						.toString());
			}
			if (model.getEcyt_gshang() == 1) {
				Label lab6 = new Label();
				lab6.setParent(gdr);
				lab6.setValue(df.format(ecycmodellist.get(m).getEcyc_sb_base())
						.toString());
			}
			if (model.getEcyt_yliao() == 1) {
				Label lab7 = new Label();
				lab7.setParent(gdr);
				lab7.setValue(df.format(ecycmodellist.get(m).getEcyc_sb_base())
						.toString());
			}
			if (model.getEcyt_sye() == 1) {
				Label lab8 = new Label();
				lab8.setParent(gdr);
				lab8.setValue(df.format(ecycmodellist.get(m).getEcyc_sb_base())
						.toString());
			}
			if (model.getEcyt_syu() == 1) {
				Label lab9 = new Label();
				lab9.setParent(gdr);
				lab9.setValue(df.format(ecycmodellist.get(m).getEcyc_sb_base())
						.toString());
			}
			...
			if (model.getEcyt_gjj() == 1) {
				...
				lab91.setValue(df.format(
						ecycmodellist.get(m).getEcyc_house_base()).toString());
				...
				lab94.setValue(df.format(
						ecycmodellist.get(m).getEcyc_house_hj()).toString());
			}
			...
			if (model.getEcyt_bcgjj() == 1) {
				...
				lab911.setValue(df.format(
						ecycmodellist.get(m).getEcyc_bc_base()).toString());
				...
				lab941.setValue(df.format(
						ecycmodellist.get(m).getEcyc_bc_total()).toString());
			...
			// lab8.addEventListener("onClick", new
			// MyListeneraudt(emsdate.get(m).getEsda_id()));

			// lab10.addEventListener("onClick", new
			// MyListeneraudt(ecycmodellist.get(m).getEcyc_id()));
			...
			lab81.addEventListener("onClick", new listener(ecycmodellist.get(m)
					.getEcyc_id()));
			// lab81.addEventListener("onClick", new listener)
			Checkbox cbone = new Checkbox();
			cbone.setParent(gdr);
			cbone.setValue(m);
			// cbone.addEventListener("onCheck", new
			// MyListenerCheckall(ecycmodellist.get(m).getEcyc_id(),gridrows));
		}
	}
	...
	public void submit() {
		...
		try {
			...
			// 生成采集单号
			// rowcout=
			// bll.adutingemcommlist(idcountstr.substring(0,idcountstr.length()-1),
	...
	public void reback(@BindingParam("self") Grid gridcols) {
		...
		for (Object obj : row) {
			...
			if (ck.isChecked() == true) {
				...
				ecycmodellist.remove(Integer.parseInt(ck.getValue()
						.toString()));
		...
		try {
			if (bll.rebacknt(ecycmodellist,0))
				{
			...
			try {
				...
				try {
					...
					WritableFont wf = new WritableFont(WritableFont.ARIAL, 12,
							...
							UnderlineStyle.NO_UNDERLINE);// 字体、粗体、斜体、下划线
					WritableCellFormat wcf = new WritableCellFormat(wf);

					setEcycmodellist(bll.searchembaselisthd(
							wtztlist.indexOf(wtzt), cbcity, wtjgname));
					...
					if (ecycmodellist.size() == 0) {
						...
						for (int m = 0; m < ecycmodellist.size(); m++) {
							// 委托方雇员编号
							label = new jxl.write.Label(1, m + 1, ecycmodellist
									.get(m).getGid().toString(), wcf);
							sheet.addCell(label);
							// 雇员姓名
							label = new jxl.write.Label(3, m + 1, ecycmodellist
									.get(m).getEmba_name().toString(), wcf);
							sheet.addCell(label);
							// 委托方公司编号
							label = new jxl.write.Label(5, m + 1, ecycmodellist
									.get(m).getCid().toString(), wcf);
							sheet.addCell(label);
							// 公司名称
							label = new jxl.write.Label(7, m + 1, ecycmodellist
									.get(m).getCoba_company().toString(), wcf);
							sheet.addCell(label);
							// 身份证号码
							label = new jxl.write.Label(8, m + 1, ecycmodellist
									.get(m).getEmba_idcard().toString(), wcf);
							sheet.addCell(label);
							// 执行城市
							label = new jxl.write.Label(9, m + 1, ecycmodellist
									.get(m).getCity().toString(), wcf);
							sheet.addCell(label);
							// //现养老企业基数
							// label = new jxl.write.Label(11,m+1,
							// df.format(ecycmodellist.get(m).getEcyc_yl_base()).toString(),
							// wcf);
							// sheet.addCell(label);
							// //现养老个人基数
							// label = new jxl.write.Label(13, m+1,
							// df.format(ecycmodellist.get(m).getEcyc_yl_base()).toString(),
							// wcf);
							// sheet.addCell(label);
							// //现养老医疗企业基数
							// label = new jxl.write.Label(17,m+1,
							// df.format(ecycmodellist.get(m).getEcyc_jl_base()).toString(),
							// wcf);
							// sheet.addCell(label);
							// //现养老个医疗人基数
							// label = new jxl.write.Label(19,m+1,
							// df.format(ecycmodellist.get(m).getEcyc_jl_base()).toString(),
							// wcf);
							// sheet.addCell(label);
							// //现失业企业基数
							// label = new jxl.write.Label(23,m+1,
							// df.format(ecycmodellist.get(m).getEcyc_sye_base()).toString(),
							// wcf);
							// sheet.addCell(label);
							// //现失业个人基数
							// label = new jxl.write.Label(25, m+1,
							// df.format(ecycmodellist.get(m).getEcyc_sye_base()).toString(),
							// wcf);
							// sheet.addCell(label);
							// //现工伤企业基数
							// label = new jxl.write.Label(29,m+1,
							// df.format(ecycmodellist.get(m).getEcyc_gs_base()).toString(),
							// wcf);
							// sheet.addCell(label);
							// //现生育企业基数
							// label = new jxl.write.Label(33,m+1,
							// df.format(ecycmodellist.get(m).getEcyc_syu_base()).toString(),
							// wcf);
							// sheet.addCell(label);

							if (model.getEcyt_ylao() == 1) {
								// 现养老企业基数
								label = new jxl.write.Label(11, m + 1, df
										.format(ecycmodellist.get(m)
												.getEcyc_sb_base()).toString(),
										wcf);
								sheet.addCell(label);
								// 现养老个人基数
								label = new jxl.write.Label(13, m + 1, df
										.format(ecycmodellist.get(m)
												.getEcyc_sb_base()).toString(),
										wcf);
								sheet.addCell(label);
							...
							if (model.getEcyt_gshang() == 1) {
								// 现工伤企业基数
								label = new jxl.write.Label(29, m + 1, df
										.format(ecycmodellist.get(m)
												.getEcyc_sb_base()).toString(),
										wcf);
								sheet.addCell(label);
							...
							if (model.getEcyt_yliao() == 1) {
								// 现养老医疗企业基数
								label = new jxl.write.Label(17, m + 1, df
										.format(ecycmodellist.get(m)
												.getEcyc_sb_base()).toString(),
										wcf);
								sheet.addCell(label);
								// 现养老个医疗人基数
								label = new jxl.write.Label(19, m + 1, df
										.format(ecycmodellist.get(m)
												.getEcyc_sb_base()).toString(),
										wcf);
								sheet.addCell(label);
							...
							if (model.getEcyt_sye() == 1) {
								// 现失业企业基数
								label = new jxl.write.Label(23, m + 1, df
										.format(ecycmodellist.get(m)
												.getEcyc_sb_base()).toString(),
										wcf);
								sheet.addCell(label);
								// 现失业个人基数
								label = new jxl.write.Label(25, m + 1, df
										.format(ecycmodellist.get(m)
												.getEcyc_sb_base()).toString(),
										wcf);
								sheet.addCell(label);
							...
							if (model.getEcyt_syu() == 1) {
								// 现生育企业基数
								label = new jxl.write.Label(33, m + 1, df
										.format(ecycmodellist.get(m)
												.getEcyc_sb_base()).toString(),
										wcf);
								sheet.addCell(label);
							...
							// 现公积金企业基数
							label = new jxl.write.Label(37, m + 1, df.format(
									ecycmodellist.get(m).getEcyc_house_base())
									.toString(), wcf);
							sheet.addCell(label);
							// 现公积金个人基数
							label = new jxl.write.Label(39, m + 1, df.format(
									ecycmodellist.get(m).getEcyc_house_base())
									.toString(), wcf);
							sheet.addCell(label);
							...
							label = new jxl.write.Label(41, m + 1,
									ecycmodellist.get(m).getEcyc_house_cp()
											.toString(), wcf);
							sheet.addCell(label);

							// 现公积金个人比例
							label = new jxl.write.Label(43, m + 1,
									ecycmodellist.get(m).getEcyc_house_op()
											.toString(), wcf);
							sheet.addCell(label);

							// 现补充公积金个人基数
							label = new jxl.write.Label(47, m + 1, df.format(
									ecycmodellist.get(m).getEcyc_bc_base())
									.toString(), wcf);
							sheet.addCell(label);
							// 现补充公积金个人基数
							label = new jxl.write.Label(49, m + 1, df.format(
									ecycmodellist.get(m).getEcyc_bc_base())
									.toString(), wcf);
							sheet.addCell(label);
							// 现补充公积金企业比例
							label = new jxl.write.Label(51, m + 1,
									ecycmodellist.get(m).getEcyc_bc_op()
											.toString(), wcf);
							sheet.addCell(label);
							// 现补充公积金个人比例
							label = new jxl.write.Label(53, m + 1,
									ecycmodellist.get(m).getEcyc_bc_cp()
											.toString(), wcf);
							sheet.addCell(label);
							
							//备注
//							label = new jxl.write.Label(56, m + 1,
//									ecycmodellist.get(m).getEcyc_remark()
//											.toString(), wcf);
//							sheet.addCell(label);

							// 更新状态为已导出
							bll.updateemcomm("("
									+ ecycmodellist.get(m).getEcyc_id() + ")",
									3);
						}
						...
						try {
							...
							e.printStackTrace();
							Messagebox.show("获取下载文件失败!", "ERROR",
