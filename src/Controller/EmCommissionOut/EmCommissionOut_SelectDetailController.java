...
public class EmCommissionOut_SelectDetailController {

	private List<EmCommissionOutChangeModel> ecocList = new ListModelList<>();
	Integer daid = 0;

	public EmCommissionOut_SelectDetailController() {
		try {
	...
	public void init() {
		...
		try {
			setEcocList(bll
					.getEmCommOutChangeList(" and typename='后道状态' and ecoc_ecou_id="
							+ daid));
		} catch (Exception e) {
			e.printStackTrace();
		}
