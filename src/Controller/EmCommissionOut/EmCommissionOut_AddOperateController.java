...
public class EmCommissionOut_AddOperateController {
	...
	private List<EmCommissionOutChangeModel> ternotcomList = new ListModelList<>();
	
	private EmCommissionOutChangeModel mm = new EmCommissionOutChangeModel();

	public EmCommissionOut_AddOperateController() {
		try {
	...
	public void init() {
		...
		try {
			...
			// 获取离职单未完成列表
			ternotcomList = bll.getEmCommOutChangeList(" and a.gid="
					+ m.getGid() + " and ecoc_addtype='离职' and ecoc_state!=6");

			if (ternotcomList.size() > 0) {
				m.setIf_submit(true);
			} else {
	...
	public void calculateFee() {
		try {
			// 设置计算信息
			calUtil.setCalculator(titleModel.getSoin_id(), m.getTitle_date(),
			...
			e.printStackTrace();
			Messagebox
					.show("费用计算出错!", "ERROR", Messagebox.OK, Messagebox.ERROR);
		}
		calSum();
	}
	 ...
	 */
	@Command("submit")
	public void submit(@BindingParam("win") Window win,@BindingParam("url_state") org.zkoss.zul.Label url_state) {
		EmCommissionOut_OperateBll bll = new EmCommissionOut_OperateBll();
				...
				if (Messagebox.show("所有实际起始日已填写,是否完成此单?", "CONFIRM",
					...
					if (str[0].equals("1")) {
						...
						Messagebox.show(str[1], "INFORMATION", Messagebox.OK,
								Messagebox.INFORMATION);
						win.detach();
						//if(url_state.getValue().equals("1")){
		...
		excel: {
			try {
				...
				try {
					...
					// 合同起始时间
					label = new Label(1, 15, m.getEcoc_compact_f(), wcf);
					sheet.addCell(label);
					
					// 合同结束时间
					label = new Label(4, 15, m.getEcoc_compact_l(), wcf);
					sheet.addCell(label);
					...
					// 公司社保费
					label = new Label(0, 25, "公司社保费："+m.getEcoc_sb_co_sum(), wcf);
					sheet.addCell(label);
					
					// 个人社保费
					label = new Label(2, 25, "个人社保费："+m.getEcoc_sb_em_sum(), wcf);
					sheet.addCell(label);
					
					// 公司公积金费
					label = new Label(4, 25, "公司公积金费："+m.getEcoc_gjj_co_sum(), wcf);
					sheet.addCell(label);
					
					// 个人公积金费
					label = new Label(6, 25, "个人公积金费："+m.getEcoc_sb_em_sum(), wcf);
					sheet.addCell(label);
	...
	public List<EmCommissionOutChangeModel> getTernotcomList() {
		return ternotcomList;
	}

	public void setTernotcomList(List<EmCommissionOutChangeModel> ternotcomList) {
		this.ternotcomList = ternotcomList;
	}
}
