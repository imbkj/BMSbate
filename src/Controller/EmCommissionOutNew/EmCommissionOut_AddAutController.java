...
public class EmCommissionOut_AddAutController {
	...
	public void init() throws SQLException {
		...
		wtout_remarklist = new ListModelList<EmCommissionOutChangeModel>(
				bll.wtout_remarklist(ecoc_id));// 获取委托备注
	}
		...
		excel: {
			try {
				...
				try {
					...
					// 档案情况..
					label = new Label(6, 15, ecoc_list.get(0).getT188(), wcf);
					sheet.addCell(label);
					// 社保情况..
					label = new Label(4, 13, ecoc_list.get(0).getT184(), wcf);
					sheet.addCell(label);
					// 住房情况..
					label = new Label(6, 13, ecoc_list.get(0).getT183(), wcf);
					sheet.addCell(label);
					// 用工情况..
					label = new Label(6, 14, ecoc_list.get(0).getT186(), wcf);
					sheet.addCell(label);
					// 劳动合同版本..
					label = new Label(1, 16, ecoc_list.get(0).getT187(), wcf);
					sheet.addCell(label);
					// 劳动合同签订方..
					label = new Label(4, 16, ecoc_list.get(0).getT185(), wcf);
					sheet.addCell(label);
					...
					label = new Label(0, 25, "公司社保费："
							+ ecoc_list.get(0).getT197(), wcf);
					sheet.addCell(label);

					// 个人社保费
					label = new Label(2, 25, "个人社保费："
							+ ecoc_list.get(0).getT196(), wcf);
					sheet.addCell(label);

					// 公司公积金费
					label = new Label(4, 25, "公司公积金费："
							+ ecoc_list.get(0).getT195(), wcf);
					sheet.addCell(label);

					// 个人公积金费
					label = new Label(6, 25, "个人公积金费："
							+ ecoc_list.get(0).getT194(), wcf);
					sheet.addCell(label);
					...
					// 社保合计
					label = new Label(0, row1, ecoc_list.get(0).getT193() + "",
							wcf);
					sheet.addCell(label);
					// 公积金合计
					label = new Label(1, row1, ecoc_list.get(0).getT192() + "",
							wcf);
					sheet.addCell(label);
					// 档案费
					label = new Label(2, row1, ecoc_list.get(0).getT191() + "",
							wcf);
					sheet.addCell(label);
					// 福利合计
					label = new Label(3, row1, ecoc_list.get(0).getT190() + "",
							wcf);
					sheet.addCell(label);
					// 服务费
					...
					sheet.addCell(label);
					// 合计
					label = new Label(6, row1, ecoc_list.get(0).getT189() + "",
							wcf);
					sheet.addCell(label);
				} catch (Exception e) {
					e.printStackTrace();
	...
	public void remark_add() throws SQLException {
		...
		Window wnd = (Window) Executions.createComponents("EmCommissionOut_Remark.zul", null, arg);
		wnd.doModal();
		wtout_remarklist = new ListModelList<EmCommissionOutChangeModel>(
				bll.wtout_remarklist(ecoc_id));// 获取委托备注
	}
