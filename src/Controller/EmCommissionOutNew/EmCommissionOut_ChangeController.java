...
import bll.EmCommissionOutNew.EmCommissionOut_ChangeBll;
...
public class EmCommissionOut_ChangeController {
	...
	EmCommissionOut_ChangeBll bll = new EmCommissionOut_ChangeBll();
	...
	public void init() throws SQLException {
		m = (EmCommissionOutModel) Executions.getCurrent().getAttribute("m");
		wt_list = bll.getwt_list(m.getEcou_id().toString());// 显示委托主表明细

		wtout_stand = new ListModelList<EmCommissionOutStandardModel>(
				bll.getwtout_stand(m.getEcou_id().toString()));// 获取委托出服务标准

		wtout_title = new ListModelList<EmCommissionOutStandardModel>(
				bll.getwtout_title(m.getEcou_ecos_id().toString()));// 获取委托出服务标准

		wtout_feedetail = new ListModelList<EmCommissionOutFeeDetailChangeModel>(
				bll.getfeedetail(m.getEcou_id().toString(), 0, "", 0, "", "",
						"", wt_list.getEcoc_bjmonth(),
						wt_list.getEcoc_bjmonth(), wt_list.getEcoc_bjmonth(),
						"1", wt_list.getWtss_shebaopayty(),
		...
		compact_f = DateStringChange.StringtoDate(wt_list.getEcoc_compact_f(),
				"yyyy-MM-dd");
		if ("无固定期限".equals(wt_list.getEcoc_compact_l())) {
			compact_l_str = "无固定期限";
			compact_l = null;
		} else if ("有固定期限".equals(wt_list.getEcoc_compact_l())) {
			compact_l_str = "有固定期限";
	...
	public void sb_baseChange(@BindingParam("sb_base") Doublebox sb_base,
		...
		try {
			...
			sb_ti = m.getEcou_soin_id();
		}

		m.setEcou_soin_id(sb_ti);

		wtout_feedetail = new ListModelList<EmCommissionOutFeeDetailChangeModel>(
				bll.getfeedetail(m.getEcou_id().toString(), sb_ti, sb_base
						.getValue().toString(), 1, house_base.getValue()
						.toString(), "", "", wt_list.getEcoc_bjmonth(), wt_list
						.getEcoc_bjmonth(), wt_list.getEcoc_bjmonth(), "1",
						wt_list.getWtss_shebaopayty(), wt_list
	...
	public void base_per_change(@BindingParam("sb_base") Doublebox sb_base,
		...
		balist = new ListModelList<EmCommissionOutFeeDetailChangeModel>(
				bll.getfeedetail(m.getEcou_id().toString(),
						m.getEcou_soin_id(),
						abase.getEofc_co_base().toString(), 1, abase
								.getEofc_co_base().toString(), "", "", abase
								.getEofc_cp(), abase.getEofc_op(), abase
								.getEofc_cp(), "2", wt_list
								.getWtss_shebaopayty(), wt_list
								.getWtss_gjjpayty()));// 显示委托费用明细
		...
		balist = new ListModelList<EmCommissionOutFeeDetailChangeModel>(
				bll.getfeedetail(m.getEcou_id().toString(),
						m.getEcou_soin_id(),
						abase.getEofc_em_base().toString(), 1, abase
								.getEofc_em_base().toString(), "", "", abase
								.getEofc_cp(), abase.getEofc_op(), abase
								.getEofc_op(), "2", wt_list
								.getWtss_shebaopayty(), wt_list
								.getWtss_gjjpayty()));// 显示委托费用明细
	...
	public void fileChange(@BindingParam("ch1") Combobox ch1)
		...
		for (int i = 0; i < wtout_feedetail.size(); i++) {
			if (ch1.getValue().equals("是")) {
				...
				setFilelist(bll.getfilelist(m.getGid(), wt_list.getCoab_id()));
				if (filelist.size() == 0) {
	...
	public void submit(@BindingParam("sb_base") Doublebox sb_base,
			...
			@BindingParam("wtss_lab") Combobox wtss_lab) throws Exception {
		int rsult = 0;
		if (remark.getValue().equals("")) {
			Messagebox.show("请在备注中录入调整内容！", "ERROR", Messagebox.OK,
					Messagebox.ERROR);
			return;
		}
		...
		// if (!wt_list.getWtss_archives().equals("不保管")
		// && ch1.getValue().equals("")) {
		// Messagebox.show("请选择是否保管档案！", "ERROR", Messagebox.OK,
		// Messagebox.ERROR);
		// return;
		// }
		...
		for (int i = 0; i < wtout_feedetail.size(); i++) {
			...
			if (!sf_date.equals("")) {
				...
				// rsult = bll.change_feeadd(m.getEcou_id(),
				// wtout_feedetail.get(i), sf_date);// 更新二次确认时间
				
				if (wtout_feedetail.get(i).getEofc_name().toString().equals("住房公积金")&&!wtout_feedetail.get(i).getEofc_co_base().toString().equals("0.00")) {
					if(wtout_feedetail.get(i).getEofc_cp().toString().equals("0")||wtout_feedetail.get(i).getEofc_op().toString().equals("0")){
		...
		if (ecou_count >= 9) {
			String[] message = ccsaBll.change(wt_list, m.getEcou_id(),
					new BigDecimal(sb_base.getValue()), new BigDecimal(
							house_base.getValue()),
					new BigDecimal(ecoc_salary.getValue()), remark.getValue(),
					m.getEcou_soin_id().toString(), wtss_lab.getValue(),
					wt_name);

			if (!message[0].equals("0")) {
				Messagebox.show("调整成功！", "操作提示",
