...
public class EmCommissionOut_AddController {
	...
	public void fileChange(@BindingParam("ch1") Combobox ch1)
		...
		for (int i = 0; i < wtout_feedetail.size(); i++) {
			if (ch1.getValue().equals("是")) {
				...
				setFilelist(bll.getfilelist(m.getGid(), wtout_stand.get(0)
						.getEcos_cabc_id()));
				if (filelist.size() == 0) {
	...
	public void submit(@BindingParam("sb_base") Doublebox sb_base,
		...
		String[] message = ccsaBll.change(wt_list, m.getEcou_id(),
				new BigDecimal(sb_base.getValue()),
				new BigDecimal(house_base.getValue()), new BigDecimal(
						ecoc_salary.getValue()), remark.getValue(), m
						.getEcou_soin_id().toString());
