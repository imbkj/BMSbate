...
public class EmCommissionOut_EditController {
	...
	public void init() throws SQLException {
		...
		wtout_feedetail = new ListModelList<EmCommissionOutFeeDetailChangeModel>(
				bll.getfeedetailedit(m.getEcou_id().toString(), 0, "", 0, "", "",
	...
	public void sb_baseChange(@BindingParam("sb_base") Doublebox sb_base,
		...
		wtout_feedetail = new ListModelList<EmCommissionOutFeeDetailChangeModel>(
				bll.getfeedetailedit(m.getEcou_id().toString(), sb_ti, sb_base
...
System.out.println(abase.getEofc_co_base().toString());
		balist = new ListModelList<EmCommissionOutFeeDetailChangeModel>(
				bll.getfeedetailedit(m.getEcou_id().toString(),
						m.getEcou_soin_id(),
	...
	public void submit(@BindingParam("sb_base") Doublebox sb_base,
			...
			@BindingParam("g2") Grid g2, @BindingParam("ch1") Combobox ch1, @BindingParam("wtss_lab") Combobox wtss_lab)
			throws Exception {
		int rsult = 0;
	...
	//	}
		...
		result = dal.Edit_WtOut(m.getEcou_id().toString(),sb_base.getValue().toString(),house_base.getValue().toString(), 
						ecoc_salary.getValue().toString(), remark.getValue(),UserInfo.getUsername(),m
						.getEcou_soin_id().toString(),wtss_lab.getValue());
		...
		for (int i = 0; i < wtout_feedetail.size(); i++) {
			...
			if(!sf_date.equals("")){
			rsult = bll.change_feeadd(m.getEcou_id(), wtout_feedetail.get(i),
					sf_date);// 更新二次确认时间
			}
		}
