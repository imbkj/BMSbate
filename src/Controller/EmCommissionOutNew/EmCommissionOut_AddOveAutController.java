...
public class EmCommissionOut_AddOveAutController {
	...
	EmCommissionOut_AddOveAutBll bll = new EmCommissionOut_AddOveAutBll();
	...
	public void yc_aut(@BindingParam("g2") Grid gridco,
		...
		for (int i = 0; i < gridco.getRows().getChildren().size(); i++) {
			org.zkoss.zul.Label ecof_id = (org.zkoss.zul.Label) gridco
					.getCell(i, 13).getChildren().get(0);
			j = j + 1;

			Datebox ecof_co_fstart = (Datebox) gridco.getCell(i, 9)
					.getChildren().get(0);
			Datebox ecof_em_fstart = (Datebox) gridco.getCell(i, 11)
					.getChildren().get(0);
			...
			try {
				...
				if (!ecof_co_fstart.getValue().equals("")
					...
					dateStr = ecof_co_fstart.getValue().toString();
					date = (Date) sdf.parse(dateStr);
					formatStr = new SimpleDateFormat("yyyy-MM-01").format(date);
				}
			} catch (Exception e) {
			...
			try {
				if (!ecof_em_fstart.getValue().equals("")
					...
					dateStr2 = ecof_em_fstart.getValue().toString();
					date2 = (Date) sdf.parse(dateStr2);
					formatStr2 = new SimpleDateFormat("yyyy-MM-01")
							.format(date2);
				}
	...
	public void dateAll2(@BindingParam("g2") Grid gridco) throws SQLException,
			ParseException {
		try {
			Datebox cp_date1 = (Datebox) gridco.getCell(0, 11).getChildren()
					.get(0);
			...
			for (int i = 1; i < gridco.getRows().getChildren().size(); i++) {
				Datebox dateot = (Datebox) gridco.getCell(i, 11).getChildren()
						.get(0);
		...
		if (outcontent.getValue().equals("")) {
			...
			try {
				...
				if (message[0] != "0") {
					...
					if (tbll.setOpName(Integer.parseInt(message[2].toString()),
							gettaprid.get(0).getCoba_client()) == 1) {

						Messagebox.show(message[1], "操作提示", Messagebox.OK,
								Messagebox.INFORMATION);
						win.detach();
					} else {
						Messagebox.show("退回失败，请联系IT部门!", "操作提示", Messagebox.OK,
								Messagebox.ERROR);
					}
				} else {
