...
import bll.EmCommissionOutNew.EmCommissionOut_OveAutBll;
...
public class EmCommissionOut_OveAutController {
	...
	EmCommissionOut_OveAutBll bll = new EmCommissionOut_OveAutBll();
	...
	public void init() throws SQLException {
		...
		//		bll.getwtout_aut_list("", "", "", "", "新增", "", "", "", ""));// 显示未确认数据
		...
		try {
			zd_url = Executions.getCurrent().getArg().get("zd_url").toString();
			zd_gid=Executions.getCurrent().getArg().get("gid").toString();
			if(!zd_url.equals("")){
				wtout_aut_list = new ListModelList<EmCommissionOutChangeModel>(
						bll.getwtout_aut_list("", zd_gid,
								"", "", "",
								"","", "",
								""));// 显示未确认数据
			}
	...
	public void wt_aut(@BindingParam("emco") EmCommissionOutChangeModel emco,
			...
			@BindingParam("state") Combobox state) throws WrongValueException,
			SQLException {
		String aut_url = "";
		Map arg = new HashMap();
		...
		if (emco.getEcoc_addtype().equals("新增")
			...
			aut_url = "/EmCommissionOutNew/EmCommissionOut_AddOveAut.zul";
		}

		if (emco.getEcoc_addtype().equals("离职")) {
			aut_url = "/EmCommissionOutNew/EmCommissionOut_DimOveAut.zul";
		}
				...
				try {
					...
					for (int i = 0; i < gridco.getRows().getChildren().size(); i++) {
						...
						if (chk1.isChecked()) {
							...
							if (addty.equals("新增") || addty.equals("调整")) {
								ci_excel = new ListModelList<EmCommissionOutZYTModel>(
										bll.ci_excel(chk1.getValue().toString()));// 显示保险在保类型
								// 插入一行
								sheet.insertRow(row);
	...
	public void search(@BindingParam("cid") Textbox cid,
			...
			@BindingParam("city") Combobox city,
			@BindingParam("client") Combobox client,
			@BindingParam("state") Combobox state, @BindingParam("b") Grid gd)
			throws SQLException {
	...
	public void updates(@BindingParam("b") Grid gridco,
		...
		for (int i = 0; i < gridco.getRows().getChildren().size(); i++) {
			try {
				...
				if (chk1.isChecked()) {
					...
					Label type_con = (Label) gridco.getCell(i, 4).getChildren()
							.get(0);
					type_st=type_con.getValue();
				}
			} catch (Exception e) {
				// TODO: handle exception
			}
		...
		System.out.println(type_st);
		String url="";

		Map arg = new HashMap();
		arg.put("ecoc_id", chkall);
		
		if (type_st.equals("新增")||type_st.equals("调整")){
			url="/EmCommissionOutNew/EmCommissionOut_AllEc.zul";
		}else {
			url="/EmCommissionOutNew/EmCommissionOut_AllEcDi.zul";
		}
	...
	public void allback(@BindingParam("b") Grid gridco,
		...
		Window wnd = (Window) Executions.createComponents(
				"/EmCommissionOutNew/EmCommissionOut_AllBack.zul", null, arg);
		wnd.doModal();
