...
public class EmZYT_AddNewAllController {
	...
	CoCompactGroupingBll compactModel;
	...
	public EmZYT_AddNewAllController() {
		// 联系方式下拉框
	...
	// 查询公司合同
	@Command("searchCoco")
	@NotifyChange({ "cflList", "cklist", "client", "compactModel" })
	public void searchCoco(@BindingParam("cid") Intbox tbcid) {
		...
		if (cid != 0) {
			...
			try {
				String str = " and a.cid=" + String.valueOf(cid);
				client = cbll.getCobaseinfo(str).get(0).getCoba_client();
			} catch (Exception e) {
				client = "";
			}
		} else {
			Messagebox.show("请输入公司编号", "操作提示", Messagebox.OK, Messagebox.ERROR);
			return;
		}
	}
	...
	public void cancel(Grid gd, Checkbox ck) {
		if (!ck.isChecked()) {
			/*
			 * Row row = (Row) ck.getParent().getParent(); Datebox db1 =
			 * (Datebox) gd.getCell(row.getIndex(), 5).getChildren()
			 * .get(0).getChildren().get(0); Datebox db2 = (Datebox)
			 * gd.getCell(row.getIndex(), 6).getChildren()
			 * .get(0).getChildren().get(0); db1.setValue(null);
			 * db2.setValue(null);
			 */
		}
	}
				...
				public void onEvent(ClickEvent event) throws Exception {
					if (Messagebox.Button.OK.equals(event.getButton())) {
						...
						for (int i = 0; i < emztList.size(); i++) {
							...
							String[] message = obll.addEmZYTCoGlist(emztList
									.get(i).getId(), intime, id,
									emzt_contacttype.getValue(), username);
	...
	public void setCompactModel(CoCompactGroupingBll compactModel) {
