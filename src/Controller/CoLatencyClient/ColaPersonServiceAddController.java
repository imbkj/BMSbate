...
public class ColaPersonServiceAddController {
	...
	public ColaPersonServiceAddController() {
		...
		if(spb.getmodellist(Integer.parseInt(cid)).size()>0)
		{
			cosalarysetm = spb.getmodellist(Integer.parseInt(cid)).get(0);
		}
	}
	...
	public void createwin(@BindingParam("win") Window win) {
		if (servicemodel.getCsqe_request() == 1) {
			Messagebox.show("该公司已有人事服务要求信息", "提示", Messagebox.OK,
					Messagebox.ERROR);
			reUrl();
		}
	}
	...
	public void addrequest(@BindingParam("win") Window win) {
		...
		if (k > 0) {
			...
			if (list.size() > 0) {
				servicemodel.setCsqe_request(1);
