...
public class ColaSalaryInfoEditController {
	...
	public ColaSalaryInfoEditController() {
	...
	public void createwin(@BindingParam("win") Window win) {
		if (servicemodel.getCsqe_salary() != 1) {
			Messagebox.show("该公司还没有薪酬服务信息", "提示", Messagebox.OK,
					Messagebox.ERROR);
			win.detach();
		}
	}
	...
	public void setvisble(@BindingParam("grid1") Grid gdzz,
		...
		if (cm.getValue().equals("E-mail工资单")) {
			...
			cosalarysetm.setCoss_emailouto(0);// 设置自动发送工资单为否
		} else {
			gdzz.setVisible(false);
			gdemail.setVisible(false);
	...
	// 薪酬服务信息修改
	@Command
	public void salaryEdit(@BindingParam("win") Window win) {
