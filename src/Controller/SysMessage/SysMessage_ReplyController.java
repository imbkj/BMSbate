...
public class SysMessage_ReplyController {
	...
	private List<LoginModel> replyList = new ListModelList<LoginModel>();
	...
	// 初始化
	public SysMessage_ReplyController() throws Exception {
		...
		userid = user.getUserid().equals("err") ? 0 : Integer.parseInt(user
				.getUserid());
		setLgModel(rbll.getSenderModel(smModel.getSyme_id()));
		selectloginliList.add(lgModel);
		String str = "";
		String str1 = "";
		...
		setReplyList(rbll.getReplyList(smModel.getSyme_id(), userid));
	...
	public void check() {
		if (ischeck) {
			selectloginliList.addAll(replyList);
			loginList.removeAll(replyList);
		} else {
			selectloginliList.removeAll(replyList);
			loginList.addAll(replyList);
		}
	}
	...
	public void send(@BindingParam("win") Window win) throws Exception {
		if (selectloginliList.size() <= 0) {
			...
			issuccess += bll.updateReplyState(smModel.getSymr_id(), 1);
			if (issuccess == 2) {
	...
	public int submit(int state) throws Exception {
		...
		model.setSyme_reply_id(smModel.getSyme_id());
	...
	public LoginModel getLgModel() {
		return lgModel;
	}

	public void setLgModel(LoginModel lgModel) {
	...
	public List<LoginModel> getReplyList() {
		return replyList;
	}

	public void setReplyList(List<LoginModel> replyList) {
