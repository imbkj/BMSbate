...
public class Finance_HouseBjGatListController {
	...
	String ownmonths = "", ifsingle = "0";
	private String zztotal = "0.0";

	public Finance_HouseBjGatListController() {
		...
		list = bll.getHhouseBjGatherList("", ownmonths, ifsingle);
		getTotalInfo(ownmonths);
	}

	// 计算中智户和委托户的扣款总额
	private void getTotalInfo(String ownmonth) {
		BigDecimal s[] = bll.getHsBjTotalInfo(ownmonth);
	...
	public void search(@BindingParam("ownmonth") Date ownmonth) {
		...
		getTotalInfo(ownmonths);
		list = bll.getHhouseBjGatherList(sql, ownmonths, ifsingle);
	}
	...
	public void addoffer() {
		...
		Window window = (Window) Executions.createComponents(
				"Finance_HouseBjCreateTxt.zul", null, map);
		window.doModal();
	}
	...
	public void OutExcel() throws Exception {
		...
		// 格式化日期(产生文件名)
		String filename = "公积金补缴分录" + sdf.format(date) + ".xls";// 定义导出文件名称
		// 获取绝对路径
		...
		file.createNewFile();
		String sheetName = "公积金补缴台账";// Excel表格名
		// 定义表头
		String[] title = { "序号","委托机构", "所属月份", "用友编号", "客户类型", "科目代码",
				"公司编号", "公司名称", "公积金补缴金额", "公积金收款金额", "差额", "客服", "到账情况" };
		try {
			// 使用自己写的Excel导出实现类把数据写入Excel
			ExcelService exl = new Finance_HouseBjOutExcelImpl(file, sheetName,
					title, list);
			exl.writeExcel();
		} catch (Exception e) {
