...
public class Finance_HouseOutExcelImpl implements ExcelService {
	private File file;// excel文件对象
	private String sheetName;// sheet名称
	private String[] title;// 标题字符串
	private List<SbGatherModel> bclist;// excel中的内容
	public Finance_HouseOutExcelImpl(File file, String sheetName, String[] title,
	...
	public void writeExcel() throws Exception {
		...
		for (int row = 1; row < bclist.size() + 1; row++) {
			...
			sheet.addCell(label);
			String hstotal="";
			if(m.getHstotal()!=null)
			{
				hstotal=m.getHstotal().toString();
			}
			label = new Label(8, row, hstotal);
			sheet.addCell(label);
			...
			BigDecimal sunTotal=m.getHstotal().subtract(m.getCfsa_total());
