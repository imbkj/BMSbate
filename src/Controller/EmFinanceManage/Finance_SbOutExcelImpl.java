...
public class Finance_SbOutExcelImpl implements ExcelService{
	private File file;// excel文件对象
	private String sheetName;// sheet名称
	private String[] title;// 标题字符串
	private List<SbGatherModel> bclist;// excel中的内容
	public Finance_SbOutExcelImpl(File file, String sheetName, String[] title,
	...
	public void writeExcel() throws Exception {
		...
		for (int row = 1; row < bclist.size() + 1; row++) {
			...
			label = new Label(2, row, m.getOwnmonth().toString());//第row行第1列
			sheet.addCell(label);
			...
			sheet.addCell(label);
			String sbktotal="";
			if(m.getSbktotal()!=null)
			{
				sbktotal=m.getSbktotal().toString();
			}
			label = new Label(8, row, sbktotal);
			sheet.addCell(label);
			String sbbjtotal="";
			if(m.getSbbjtotal()!=null)
			{
				sbbjtotal=m.getSbbjtotal().toString();
			}
			label = new Label(9, row,sbbjtotal);
			sheet.addCell(label);
			String cfsa_total="";
			...
			label = new Label(10, row, cfsa_total);
			sheet.addCell(label);
			BigDecimal sunTotal=BigDecimal.ZERO;
			if(m.getSbktotal()==null)
			{
				m.setSbktotal(BigDecimal.ZERO);
			}
			if(m.getCfsa_total()==null)
			{
				m.setCfsa_total(BigDecimal.ZERO);
			}
			m.getSbktotal().subtract(m.getCfsa_total());
			label = new Label(11, row, sunTotal.toString());
			sheet.addCell(label);
			label = new Label(12, row, m.getCoba_client());
			sheet.addCell(label);
			label = new Label(13, row,"已托收");
			sheet.addCell(label);
		}
		// 写入数据
		wwb.write();
