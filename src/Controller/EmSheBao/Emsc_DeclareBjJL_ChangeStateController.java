...
public class Emsc_DeclareBjJL_ChangeStateController {
	...
	public Emsc_DeclareBjJL_ChangeStateController() {

		selbll = new Emsc_DeclareSelBll();
		bjModel = selbll.getBjJLInfoById(id);
		...
		msgservice = new MessageImpl("EmShebaoBJJL", id);
	...
	public void upState(@BindingParam("state") Radiogroup rgState,
		...
		try {
			...
			if ("".equals(message)) {
				...
				String[] mes = opbll.BjJLDeclareUpState(bjModel,
	...
	public void addFlag(@BindingParam("flag") Textbox txtFlag) {
		if (!"".equals(txtFlag.getValue())) {
			if (!txtFlag.getValue().equals(bjModel.getEmsb_flag())) {
				...
				int i = bll.UpJLFlag(bjModel, UserInfo.getUsername());
				if (i == 1) {
