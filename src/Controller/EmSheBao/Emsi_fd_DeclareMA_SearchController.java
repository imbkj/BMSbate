...
public class Emsi_fd_DeclareMA_SearchController {
	...
	public Emsi_fd_DeclareMA_SearchController() {
		// 客服列表
	...
	public void search(@BindingParam("win") Window win,
			...
			@BindingParam("fd_ctime") Datebox fd_ctime) {

		String sql = " and escm_fee>0";
		...
		if (!"".equals(keyword.getValue())) {
			...
			Window window = (Window) Executions.createComponents(
					"Emsi_fd_DeclareMA_List.zul", null, map);
			window.doModal();
			...
			if (s_ownmonth.getSelectedItem() != null) {
				...
				if (!s_state.getSelectedItem().getValue().equals("10")) {
					if (!s_state.getSelectedItem().getValue().equals("19")) {
						sql = sql + " AND escm_ifdeclare>=12";
					} else {
						sql = sql + " AND escm_ifdeclare=11";//已传决定书(未确认收款)
					}
				}
				...
				//到账日期
				// 委托单日期范围
				String fd_ctime_String = "";
				try {
					if (fd_ctime.getValue() != null) {
						fd_ctime_String = dsc.DatetoSting(fd_ctime.getValue(),
								"yyyy-MM-dd");
						sql = sql + "  AND datediff(d,escm_fd_ctime,'"+fd_ctime_String+"')=0";
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
