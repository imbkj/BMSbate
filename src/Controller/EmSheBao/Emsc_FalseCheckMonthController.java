...
public class Emsc_FalseCheckMonthController {
	...
	public Emsc_FalseCheckMonthController() {
		// 获取最新月份数据
		sbDataList = dsbll.getShebaoErrMonth(nowmonth);
	}
	...
	public void search(@BindingParam("s_ownmonth") Combobox s_ownmonth) {
		if (s_ownmonth.getSelectedItem() != null) {
			sbDataList = dsbll.getShebaoErrMonth(s_ownmonth.getSelectedItem()
					.getLabel());
		}
	}
	...
	public void edit(@BindingParam("model") EmShebaoErrModel m,
		...
		Window window = (Window) Executions.createComponents(
				"Emsc_FalseCheckMonthMsg.zul", null, map);
		window.doModal();
		// 刷新
		sbDataList = dsbll.getShebaoErrMonth(s_ownmonth.getSelectedItem()
				.getLabel());
	...
	public void info(@BindingParam("model") EmShebaoErrModel m) {
		...
		map.put("model", m);
		Window window = (Window) Executions.createComponents(
				"Emsc_FalseCheckMonthInfo.zul", null, map);
		window.doModal();
	}
	...
	public void clean(@BindingParam("s_ownmonth") Combobox s_ownmonth) {
		if (s_ownmonth.getSelectedItem() != null) {
			// 删除台账前逻辑检查数据
			String message[] = dobll.delErrSZSIMonth(s_ownmonth
