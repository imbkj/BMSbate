...
public class Agcpd_addController {
...
//			String cpst_nameStr = ply.SetToString(cpst_nameSet);
			...
			if (cop.getCopr_name() == null || cop.getCopr_name().isEmpty()) {
				...
				if (str[0].equals("1")) {
					...
					if (bll.getCoproductInfo(pid).getCopr_copc_id()==2 || bll.getCoproductInfo(pid).getCopr_copc_id()==3)
					{
						bll.insertCopComRel(pid,1);
					}
					else if (bll.getCoproductInfo(pid).getCopr_copc_id()==8) //财税
					{
						bll.insertCopComRel(pid,2);
					}
					else
					{
						bll.insertCopComRel(pid,0);
					}
