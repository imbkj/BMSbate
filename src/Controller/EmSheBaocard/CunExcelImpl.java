...
public class CunExcelImpl implements ExcelService {
	...
	public void writeExcel() throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			sheet.addCell(label);
			label = new Label(1, row, model.getSbcd_name(), getHeader());
			sheet.addCell(label);
			label = new Label(2, row, model.getSbcd_computerid(), getHeader());
			sheet.addCell(label);
			label = new Label(3, row, getCode(marylist, model.getSbcd_marry()),
					getHeader());
			sheet.addCell(label);
			label = new Label(4, row, getCode(edulist,
					model.getSbcd_education()), getHeader());
			sheet.addCell(label);
			label = new Label(5, row, model.getSbcd_mobile(), getHeader());
			sheet.addCell(label);
			label = new Label(6, row, model.getSbcd_address(), getHeader());
			sheet.addCell(label);
			row++;
			y++;
		}
