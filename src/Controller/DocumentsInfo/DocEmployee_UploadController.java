...
public class DocEmployee_UploadController {
...
//	private final int gid =1;
	...
	public DocEmployee_UploadController() {
		setClassList();
		setPicList();
	}
	...
	public void upload(@BindingParam("class") Combobox cb) throws Exception {
		...
		try {
			if (this.media != null) {
				...
				String name = path + nowtime + doin_id + gid + "."
						+ media.getFormat();
				// 上传文件
				if (FileOperate.upload(media, name)) {
					if (bll.DocFileUpload(0, gid, doin_id, name,
	...
	public void setClassList() {
		try {
			classList = bll.getDocClassByType(1);
		} catch (Exception e) {
