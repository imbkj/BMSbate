...
public class Task_FlowControlController {
	...
	public Task_FlowControlController() {
		...
		// allTaskList = bll.getAllTaskList();
		// bll.getTaskList(allTaskList, allClassList, wfClassList, wfDeList);
		// winclassList = allClassList;
	...
	// 查看可操作人
	@Command
	public void selOpName(@BindingParam("tali_id") int tali_id,
			@BindingParam("tali_name") String tali_name) {
		Map<String, String> map = new HashMap<String, String>();
		map.put("tali_id", String.valueOf(tali_id));
		map.put("tali_name", String.valueOf(tali_name));
		Window window = (Window) Executions.createComponents(
				"Task_SelOpName.zul", null, map);
		window.doModal();
	}

	// 流程调整
	@Command
	public void controlNode(@BindingParam("tali_id") int tali_id,
			@BindingParam("tali_name") String tali_name) {
		Map<String, String> map = new HashMap<String, String>();
		map.put("tali_id", String.valueOf(tali_id));
		map.put("tali_name", String.valueOf(tali_name));
		Window window = (Window) Executions.createComponents(
				"Task_controlNode.zul", null, map);
		window.doModal();
	}

	// 终止流程
	@Command
	public void stopList(@BindingParam("tali_id") int tali_id,
			@BindingParam("tali_name") String tali_name) {
		Map<String, String> map = new HashMap<String, String>();
		map.put("tali_id", String.valueOf(tali_id));
		map.put("tali_name", String.valueOf(tali_name));
				...
				if (searchKey != null && !searchKey.equals("")) {
					if (searchValue != null && !searchValue.equals("")) {
						...
						case "员工编号":
							...
							break;
						case "所属月份":
							ownmonth = searchValue;
							break;
						case "任务单编号":
							taliId = searchValue;
							break;
						}
						allTaskList = bll.allTaskList3(cid, gid, ownmonth,
								taliId);
						searTaskList = thisTaskList = allTaskList;
						for (TaskListModel m : allTaskList) {
							list.add(m);
						}
						wininfoList = thisTaskList = list;
						// 重新设置任务单类型
	...
	public void search() {
		...
		} else if ((wfClass != null && !wfClass.equals(""))) {
			winclassList = bll.wfClassList2(wfClass);
			allTaskList = bll.allTaskList2(wfClass, null);
			searTaskList = thisTaskList = allTaskList;
