...
public class Embase_DetailInfoController {
	...
	// 页面传值
	private Integer emba_id = 0;
	private EmZYTModel emztM;

	// 初始化
	public Embase_DetailInfoController() {
		try {
		...
		try {
			...
			// cid = ((EmZYTModel)
			// Executions.getCurrent().getArg().get("emztM"))
			// .getCid();
		} catch (Exception e) {
			// TODO: handle exception
		}
		...
		try {
			...
			csidList = selectbll.getcsidList(" AND cid=" + cid); // 职位
			cpidList = selectbll.getcpidList(" AND cid=" + cid);// 部门
			...
			else if(m.getEmba_state()==1)
			{
				state_name="在职";
			}
			else if(m.getEmba_state()==2||m.getEmba_state()==3||m.getEmba_state()==5)
			{
				state_name="入职中";
			}else if(m.getEmba_state()==4)
			{
				state_name="取消入职";
			}
			// 调用智翼通接口数据携带过来方法
	...
	public void fieldinit() {
		try {
			...
			if (m.getEmba_fileinclass() != null) {
				fileinclass = m.getEmba_fileinclass() == 0 ? "否" : "是";
			}
			if (m.getEmba_filedebts() != null) {
				filedebts = m.getEmba_filedebts() == 0 ? "否" : "是";
			}
			if (m.getEmba_filehj() != null) {
				filehj = m.getEmba_filehj() == 0 ? "否" : "是";
			}
			if (m.getEmba_excompanystate() != null) {
			...
			if (m.getEmba_csid() != null) {
				for (int i = 0; i < csidList.size(); i++) {
					if (csidList.get(i).getCoba_positionid() == m
							.getEmba_csid()) {
			...
			if (m.getEmba_cpid() != null) {
				for (int i = 0; i < cpidList.size(); i++) {
					if (cpidList.get(i).getCoba_pid() == m.getEmba_cpid()) {
	...
	public boolean check() throws Exception {
		...
		try {
			for (int i = 0; i < requiredList.size(); i++) {
				...
				if (fname != null && fname.equals("emba_degree")) {
				...
				} else if (fname != null && fname.equals("emba_ep")) {
				...
				} else if (fname != null && fname.equals("emba_hospital")) {
				...
				} else if (fname != null && fname.equals("emba_gzbank")) {
				...
				} else if (fname != null && fname.equals("emba_sbbank")) {
				...
				} else if (fname != null && fname.equals("emba_sbinfor")) {
					Object a = "";
					Object b = "";
					Object c = "";
					Object d = "";
				...
				} else if (fname != null && fname.equals("emba_workinfor")) {
					Object a = "";
					Object b = "";
					Object c = "";
					boolean su1 = true;
	...
	public void ifcomputerid() {
		try {
			if (m.getEmba_ifcomputerid() != null
					&& m.getEmba_ifcomputerid().equals("是")) {
				m.setVis_computerid(true);
				m.setVis_hand(false);
			} else if (m.getEmba_ifcomputerid() != null
					&& m.getEmba_ifcomputerid().equals("否")) {
	...
	public void issbcardnotice() {
		try {
			if (m.getEmba_sbcard() != null && m.getEmba_sbcard().equals("是")) {
