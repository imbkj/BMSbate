 ...
 * @param args
 */
public class Cfma_CollectListsController {
	...
	private List<CollectAmountModel> samountlist= new ArrayList<CollectAmountModel>();
	...
	public Cfma_CollectListsController() {
	...
	public void init() {
		...
		clientnameList = csb.getClientnameList();
		
		amountlist=csb.getCoinfoListall();
		
		//samountlist=csb.getCoinfoListall();
	...
	public void search(@BindingParam("coin") String coin) {
		// // 点击查询，先根据条件刷新列表
...
//				remark);
		...
		if (amountlist != null && amountlist.size() > 0) {
			samountlist.clear();
			for (CollectAmountModel m : amountlist) {
				if (company != null) {
					try {
				...
				if (client != null) {
					if (!RegexUtil.isExists(client, m.getCoba_client())) {
						continue;
					}
				}
...
//	}
	...
	public List<CollectAmountModel> getSamountlist() {
		return samountlist;
	}

	public void setSamountlist(List<CollectAmountModel> samountlist) {
