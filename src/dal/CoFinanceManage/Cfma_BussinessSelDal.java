...
public class Cfma_BussinessSelDal {
	...
	public List<EmFinanceSheBaoModel> getShebaoList(int cid, int ownmonth,
		...
		sql.append("select efba.gid,efba.efba_emba_name,ef.* from EmFinanceSheBao ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efsb_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efsb_cfmb_number=? and ef.efsb_state=1");
		sql.append(" union all ");
		sql.append("select efba.gid,efba.efba_emba_name,ef.* from EmFinanceSheBaoHistory ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efsb_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efsb_cfmb_number=? and ef.efsb_state=1");
		PreparedStatement psmt = conn.getpre(sql.toString());

		try {
			psmt.setInt(1, cid);
			psmt.setInt(2, ownmonth);
			psmt.setString(3, cfmb_number);
			psmt.setInt(4, cid);
			psmt.setInt(5, ownmonth);
			psmt.setString(6, cfmb_number);
			...
			while (rs.next()) {
				...
				m.setIdcard(rs.getString("efsb_esiu_idcard"));
				m.setEfsb_esiu_ownmonth(rs.getInt("efsb_esiu_ownmonth"));
	...
	public List<EmFinanceHouseGjjModel> getGjjList(int cid, int ownmonth,
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select efba.gid,efba.efba_emba_name,ef.* from EmFinanceHouseGjj ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efhg_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efhg_cfmb_number=? and ef.efhg_state=1");
		sql.append(" union all ");
		sql.append("select efba.gid,efba.efba_emba_name,ef.* from EmFinanceHouseGjjHistory ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efhg_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efhg_cfmb_number=? and ef.efhg_state=1");
		PreparedStatement psmt = conn.getpre(sql.toString());
		try {
			psmt.setInt(1, cid);
			psmt.setInt(2, ownmonth);
			...
			while (rs.next()) {
				...
				m.setEmba_name(rs.getString("efba_emba_name"));
				m.setOwnmonth(rs.getInt("ownmonth"));
				m.setEfhg_emhu_ownmonth(rs.getInt("efhg_emhu_ownmonth"));
				...
				m.setIdcard(rs.getString("efhg_emhu_idcard"));
	...
	public List<EmFinanceTaxModel> getTaxList(int cid, int ownmonth,
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select ef.*,efba.efba_emba_name from EmFinanceTax ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efta_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efta_cfmb_number=? and ef.efta_state=1");
		sql.append(" union all ");
		sql.append("select ef.*,efba.efba_emba_name from EmFinanceTaxHistory ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efta_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efta_cfmb_number=? and ef.efta_state=1");
		PreparedStatement psmt = conn.getpre(sql.toString());
		try {
			psmt.setInt(1, cid);
			psmt.setInt(2, ownmonth);
	...
	public List<EmFinanceSalaryModel> getSalaryList(int cid, int ownmonth,
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select efba.gid,efba.efba_emba_name,ef.* from EmFinanceSalary ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efsa_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efsa_cfmb_number=? and ef.efsa_state=1");
		sql.append(" union all ");
		sql.append("select efba.gid,efba.efba_emba_name,ef.* from EmFinanceSalaryHistory ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efsa_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efsa_cfmb_number=? and ef.efsa_state=1");
		PreparedStatement psmt = conn.getpre(sql.toString());
		try {
			psmt.setInt(1, cid);
			psmt.setInt(2, ownmonth);
	...
	public List<CoFinanceProductModel> getCoProductList(int cid, int ownmonth,
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select cfpr_copr_name,cfpr_Receivable from CoFinanceProduct where cfpr_state=1 and cid=? and ownmonth=? and cfpr_cfmb_number=?");
		sql.append(" union all ");
		sql.append("select cfpr_copr_name,cfpr_Receivable from CoFinanceProductHistory where cfpr_state=1 and cid=? and ownmonth=? and cfpr_cfmb_number=?");
	...
	public List<EmFinanceProductModel> getEmProductList(int cid, int ownmonth,
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select ef.efpr_id,ef.ownmonth,efba.gid,efba.efba_emba_name,ef.efpr_cpac_name,ef.efpr_copr_name,ef.efpr_Receivable,efba_emba_idcard from EmFinanceProduct ef inner join (select efba_id,gid,cid,efba_emba_name,efba_emba_idcard from EmFinanceBase) efba on ef.efpr_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efpr_cfmb_number=? and ef.efpr_state=1");
		sql.append(" union all ");
		sql.append("select ef.efpr_id,ef.ownmonth,efba.gid,efba.efba_emba_name,ef.efpr_cpac_name,ef.efpr_copr_name,ef.efpr_Receivable,efba_emba_idcard from EmFinanceProductHistory ef inner join (select efba_id,gid,cid,efba_emba_name,efba_emba_idcard from EmFinanceBase) efba on ef.efpr_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efpr_cfmb_number=? and ef.efpr_state=1");
		sql.append(" order by efba.gid");
		System.out.println(sql.toString());
		PreparedStatement psmt = conn.getpre(sql.toString());
		List<EmFinanceProductItemListModel> itemList = getEmProductItemList(
				cid, ownmonth, cfmb_number);
		try {
			psmt.setInt(1, cid);
			psmt.setInt(2, ownmonth);
			psmt.setString(3, cfmb_number);
			...
			while (rs.next()) {
				if (rs.getInt("gid") != gid) {
					...
					m.setGid(rs.getInt("gid"));
					m.setOwnmonth(rs.getInt("ownmonth"));
					m.setIdcard(rs.getString("efba_emba_idcard"));
					m.setEmba_name(rs.getString("efba_emba_name"));
	...
	public List<EmFinanceProductItemListModel> getEmProductItemList(int cid,
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select distinct efpr_cpac_name,efpr_copr_name from EmFinanceProduct ef  inner join (select efba_id,cid from EmFinanceBase) efba on ef.efpr_efba_id=efba.efba_id where  efba.cid=? and ef.ownmonth=? and ef.efpr_cfmb_number=? and ef.efpr_state=1");
		sql.append(" union all ");
		sql.append("select distinct efpr_cpac_name,efpr_copr_name from EmFinanceProductHistory ef  inner join (select efba_id,cid from EmFinanceBase) efba on ef.efpr_efba_id=efba.efba_id where  efba.cid=? and ef.ownmonth=? and ef.efpr_cfmb_number=? and ef.efpr_state=1");
	...
	public List<EmFinanceCommissionOutDetailItemModel> getEmCommissionOutDetailItem(
			int cid, int ownmonth, String cfmb_number, String city) {
		...
		sql.append(" where efcd.efco_state=1 and efba.cid=? and ef.ownmonth=? and ef.efco_cfmb_number=? and ppc.name=?");
		sql.append(" union all ");
	...
	public List<EmFinanceCommissionOutDetailItemModel> getEmCommissionOutProductItem(
		...
		sql.append(" left join CoAgencyBaseCityRel cabc on ef.efco_cabc_id=cabc.cabc_id");
		sql.append(" left join PubProCity ppc on cabc.cabc_ppc_id=ppc.id");
		sql.append(" where efcp.efcp_state=1 and efba.cid=? and ef.ownmonth=? and ef.efco_cfmb_number=? and ppc.name=?");
		sql.append(" union all ");
	...
	public List<SocialInsuranceClassInfoViewModel> getEmCommissionOutSocialInsurance(
			String cfmb_number, int cid, int ownmonth) {
	...
	// 获取委托出数据的社保标准
	public List<SocialInsuranceClassInfoViewModel> getEmShebaoSocialInsurance(
			String cfmb_number) {
		List<SocialInsuranceClassInfoViewModel> list = new ArrayList<SocialInsuranceClassInfoViewModel>();
		SocialInsuranceClassInfoViewModel m = null;
		try {
			ResultSet rs = conn
					.GRS("exec EmFinanceShebaoSocialInsurance_p_lwj " + "'"
							+ cfmb_number + "'");
			...
			while (rs.next()) {
				...
				m.setSicl_payunit(rs.getString("sicl_payunit"));
				m.setSiai_proportion(rs.getString("siai_proportion"));
				m.setSiai_basic_u(rs.getString("siai_basic_u"));
				m.setSiai_basic_d(rs.getString("siai_basic_d"));
				list.add(m);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<EmFinanceDisposableModel> getEmDisposableList(int cid,
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select efba.gid,efba.efba_emba_name,ef.efdi_cfmb_number,ef.efdi_Reason,ef.efdi_Receivable from EmFinanceDisposable ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efdi_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efdi_cfmb_number=? and ef.efdi_state=1");
		sql.append(" union all ");
		sql.append("select efba.gid,efba.efba_emba_name,ef.efdi_cfmb_number,ef.efdi_Reason,ef.efdi_Receivable from EmFinanceDisposableHistory ef inner join (select efba_id,gid,cid,efba_emba_name from EmFinanceBase) efba on ef.efdi_efba_id=efba.efba_id where efba.cid=? and ef.ownmonth=? and ef.efdi_cfmb_number=? and ef.efdi_state=1");
		PreparedStatement psmt = conn.getpre(sql.toString());
		try {
			psmt.setInt(1, cid);
			psmt.setInt(2, ownmonth);
	...
	public List<CoFinanceDisposableModel> getCoDisposableList(int cid,
		...
		StringBuilder sql = new StringBuilder();
		sql.append("select cfdi_copr_name,cfdi_Reason,cfdi_Receivable from CoFinanceDisposable where cfdi_state=1 and cid=? and ownmonth=? and cfdi_cfmb_number=?");
		sql.append(" union all ");
		sql.append("select cfdi_copr_name,cfdi_Reason,cfdi_Receivable from CoFinanceDisposableHistory where cfdi_state=1 and cid=? and ownmonth=? and cfdi_cfmb_number=?");
		PreparedStatement psmt = conn.getpre(sql.toString());
		try {
			psmt.setInt(1, cid);
			psmt.setInt(2, ownmonth);
	...
	// 获取增值税金
	public List<EmFinanceValueAddTaxModel> getvatList(int cid, int ownmonth,
			String cfmb_number) {
		List<EmFinanceValueAddTaxModel> list = new ListModelList<>();
		dbconn db = new dbconn();
		String sql = "select a.ownmonth,efvt_efba_id,a.cid,a.gid,emba_name,"
				+ "sum(efvt_Receivable)efvt_Receivable"
				+ " from EmFinanceValueAddTax a inner join embase b on a.gid=b.gid"
				+ " where a.cid=? and ownmonth=? and efvt_cfmb_number=? and efvt_state=1"
				+ " group by a.ownmonth,efvt_efba_id,a.cid,a.gid,emba_name" +
				" order by cid,emba_name";
		try {
			list = db.find(sql, EmFinanceValueAddTaxModel.class, null, cid,
					ownmonth, cfmb_number);
		} catch (SQLException e) {
