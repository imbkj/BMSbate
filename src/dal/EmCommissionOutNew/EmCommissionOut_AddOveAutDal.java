...
public class EmCommissionOut_AddOveAutDal {
	// 获取委托主表明细
	public EmCommissionOutChangeModel getwt_list(String ecoc_id)
		...
		try {
			...
			while (rs.next()) {
				...
				list.setEcoc_idcard(rs.getString("emba_idcard"));
	...
	public List<EmCommissionOutFeeDetailChangeModel> getfeedetail(String ecoc_id)
		...
		String sqlstr = "select eofc_ecoc_id,eofc_name,eofc_em_base,eofc_co_base,eofc_op,eofc_cp,eofc_month_sum,eofc_start_date,eofc_sicl_id,eofc_id,eofc_co_fstart_date,eofc_em_fstart_date,eofc_co_sum,eofc_em_sum from EmCommissionOutFeeDetailChange where eofc_ecoc_id="
				+ ecoc_id
		...
		try {
			...
			while (rs.next()) {
				...
				// model.setEofc_co_fstart_date(rs.getString("eofc_co_fstart_date"));
				// model.setEofc_em_fstart_date(rs.getString("eofc_em_fstart_date"));
				model.setTempDate(rs.getDate("eofc_co_fstart_date"));
				model.setTempDate1(rs.getDate("eofc_em_fstart_date"));
	...
	public List<EmCommissionOutFeeDetailChangeModel> getflfeedetail(
		...
		ResultSet rs = null;
		List<EmCommissionOutFeeDetailChangeModel> list = new ArrayList<EmCommissionOutFeeDetailChangeModel>();
		String sqlstr = "select eofc_ecoc_id,eofc_name,eofc_em_base,eofc_co_base,eofc_op,eofc_cp,eofc_month_sum,eofc_start_date,eofc_sicl_id,eofc_id,eofc_co_fstart_date,eofc_em_fstart_date from EmCommissionOutFeeDetailChange where eofc_ecoc_id="
				+ ecoc_id
	...
	public int getstateover(String ecoc_id, String co_date, String em_date) {
		try {
			CallableStatement c = conn
					.getcall("EmCommissionOut_NewAutUpdate_P_zzq(?,?,?,?)");
	...
	// 委托单二次确认
		public int getstateovernew(String wt_name) {
			try {
				CallableStatement c = conn
						.getcall("EmCommissionOut_NewAutUpdateN_P_zzq(?,?)");

				c.setString(1, wt_name);
