...
public class EmCommissionOut_AddAutDal {
	// 获取委托主表明细
	...
	public List<EmCommissionOutFeeDetailChangeModel> getfeedetail(String ecoc_id)
		...
		String sqlstr = "select eofc_ecoc_id,eofc_name,eofc_em_base,eofc_co_base,eofc_op,eofc_cp,eofc_month_sum,eofc_start_date,eofc_sicl_id,eofc_co_sum,eofc_em_sum from EmCommissionOutFeeDetailChange where eofc_ecoc_id="
				+ ecoc_id
				+ "  and (eofc_name='养老保险' or eofc_name='医疗保险' or eofc_name='大病医疗' or eofc_name='生育保险' or eofc_name='工伤保险' or eofc_name='失业保险' or eofc_name='住房公积金' or eofc_name='补充公积金' or eofc_name='服务费' or eofc_name='档案费')  and eofc_name not in (select '补充公积金' from EmCommissionOutFeeDetailChange where eofc_ecoc_id="+ecoc_id+" and eofc_name='补充公积金' and eofc_month_sum=0) order by eofc_sicl_id";
		try {
			System.out.println(sqlstr);
			rs = db.GRS(sqlstr);
			while (rs.next()) {
				...
				model.setEofc_start_date(rs.getString("eofc_start_date"));
	...
	public List<EmCommissionOutFeeDetailChangeModel> getqdflfeedetail(
		...
		ResultSet rs = null;
		List<EmCommissionOutFeeDetailChangeModel> list = new ArrayList<EmCommissionOutFeeDetailChangeModel>();
		String sqlstr = "select eofc_ecoc_id,eofc_name,eofc_em_base,eofc_co_base,eofc_op,eofc_cp,eofc_month_sum,eofc_start_date,eofc_sicl_id from EmCommissionOutFeeDetailChange where eofc_ecoc_id="
				+ ecoc_id
	...
	public EmCommissionOutChangeModel getqdwt_list(String ecoc_id)
		...
		ResultSet rs = null;
		EmCommissionOutChangeModel list = new EmCommissionOutChangeModel();
		String sqlstr = "select a.gid,a.cid,emba_name,coba_company,wtss_city,ecou_phone,ecou_mobile,ecou_sb_co_sum+ecou_gjj_co_sum co_to,ecou_sb_em_sum+ecou_gjj_em_sum em_to,ecou_gjj_sum,ecou_sb_sum,ecou_email,ecou_idcard,emba_sex,ecou_compact_f,ecou_compact_l,ecou_sum from EmCommissionOutChange a left join embase b on b.gid=a.gid left join cobase c on c.cid=a.cid left join wtoutFee d on d.Wtot_feeid=a.ecoc_ecos_id left join EmCommissionOut e on e.ecou_id=a.ecoc_ecou_id where ecoc_id="
				+ ecoc_id;
		try {
			System.out.println(sqlstr);
			rs = db.GRS(sqlstr);
	...
	public List<EmCommissionOutZYTModel> getci_excel(String ecoc_id)
		...
		String sqlstr = "select a.gid t1,emba_name t2,isnull(emba_sex,0) t3,isnull(ecoc_mobile,0) t4,ecoc_phone t5,a.cid t6,coba_company t7,ecoc_phone t8,ecoc_addtype t9,"
				...
				+ "'' T176,'' T177,'' T178,'' T179,'' T180,'' T181,'' T182,d.wtss_gjjco T183,d.wtss_shebaoco T184,d.wtss_laborcontract T185,d.wtss_employment T186,d.wtss_compacttype T187,d.wtss_archives T188,ecoc_sum T189,"
				+ "ecoc_welfare_sum T190,ecoc_file_fee T191,ecoc_gjj_sum T192,ecoc_sb_sum T193,ecoc_gjj_em_sum T194,ecoc_gjj_co_sum T195,ecoc_sb_em_sum T196,ecoc_sb_co_sum T197,ecoc_house_base T198,ecoc_sb_base T199,ecoc_ecos_id T200"
				...
				+ " left join WtServiceStandardrelation i on i.wtot_feeid=d.Wtot_feeid left join WtServiceStandard j on j.wtss_id=i.wtss_id"
	...
	public List<EmCommissionOutChangeModel> wtout_remarklist(String ecoc_id)
		...
		ResultSet rs = null;
		List<EmCommissionOutChangeModel> list = new ArrayList<EmCommissionOutChangeModel>();
		String sqlstr = "select ecor_content,ecor_addname,ecor_addtime from EmCommissionOutHRemark where ecor_ecoc_id="
				+ ecoc_id;
		try {
			System.out.println(sqlstr);
			rs = db.GRS(sqlstr);
			while (rs.next()) {
				EmCommissionOutChangeModel model = new EmCommissionOutChangeModel();
				model.setEcoc_addtype(rs.getString("ecor_content"));
				model.setEcoc_addname(rs.getString("ecor_addname"));
				model.setEcoc_addtime1(rs.getString("ecor_addtime"));
				list.add(model);
			}
		} catch (Exception e) {
			System.out.print(e.toString());
	...
	// 获取tapr、cid
	public int remark(String ecoc_id, String remark, String username,
			int email) throws SQLException {
		dbconn db = new dbconn();
		ResultSet rs = null;
		ResultSet rs1 = null;
		List<EmCommissionOutChangeModel> list = new ArrayList<EmCommissionOutChangeModel>();
		String sqlstr = "insert into EmCommissionOutHRemark (ecor_ecoc_id,ecor_content,ecor_addname) select "+ecoc_id+",'你处委托至'+wtss_city+'雇员-'+emba_name+'-"+remark+"','"
				+ username
				+ "' from EmCommissionOutChange a left join cobase b on b.CID=a.cid left join login c on c.log_name=b.coba_client left join embase d on d.gid=a.gid left join wtoutFee e on e.Wtot_feeid=a.ecoc_ecos_id where ecoc_id="
				+ ecoc_id;

		String sqlstr1 = "insert into PubEmail (puem_title,puem_Content,puem_Email,puem_SendTime,puem_Addname,puem_state,puem_addtime) select '委托外地未办反馈','你处委托至'+wtss_city+'雇员-'+emba_name+'-"+remark+"',log_email,GETDATE(),'"
				+ username
				+ "',0,GETDATE() from EmCommissionOutChange a left join cobase b on b.CID=a.cid left join login c on c.log_name=b.coba_client left join embase d on d.gid=a.gid left join wtoutFee e on e.Wtot_feeid=a.ecoc_ecos_id where ecoc_id="
				+ ecoc_id;
		try {
			System.out.println(sqlstr);
