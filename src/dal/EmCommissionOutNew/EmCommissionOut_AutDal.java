...
public class EmCommissionOut_AutDal {
	// 获取委托城市
	public List<EmCommissionOutChangeModel> getcitylist() throws SQLException {
		...
		String sqlstr = "select distinct wtss_city from EmCommissionOutChange a left join wtoutFee b on b.Wtot_feeid=a.ecoc_ecos_id where ecoc_state=1 and wtss_city is not null order by wtss_city";
		try {
			rs = db.GRS(sqlstr);
			while (rs.next()) {
	...
	public List<EmCommissionOutChangeModel> gettypelist(String str) throws SQLException {
		...
		ResultSet rs = null;
		List<EmCommissionOutChangeModel> list = new ArrayList<EmCommissionOutChangeModel>();
		String sqlstr = "select distinct ecoc_addtype from EmCommissionOutChange a left join wtoutfee b on b.Wtot_feeid=a.ecoc_ecos_id where ecoc_state=1"+str;
		try {
			rs = db.GRS(sqlstr);
			while (rs.next()) {
	...
	public List<EmCommissionOutChangeModel> getwtjglist(String str) throws SQLException {
		...
		ResultSet rs = null;
		List<EmCommissionOutChangeModel> list = new ArrayList<EmCommissionOutChangeModel>();
		String sqlstr = "select distinct coab_shortname from EmCommissionOutChange a left join wtoutFee b on b.Wtot_feeid=a.ecoc_ecos_id left join CoAgencyBaseCityRel c on c.cabc_id=b.coab_id left join CoAgencyBase d on d.coab_id=c.cabc_coab_id where ecoc_state=1 and coab_shortname is not null"+str+"  order by coab_shortname";
		try {
			rs = db.GRS(sqlstr);
			while (rs.next()) {
	...
	public List<EmCommissionOutChangeModel> getclientlist(String str) throws SQLException {
		...
		ResultSet rs = null;
		List<EmCommissionOutChangeModel> list = new ArrayList<EmCommissionOutChangeModel>();
		String sqlstr = "select distinct coba_client from EmCommissionOutChange a left join cobase b on b.cid=a.cid left join wtoutfee c on c.Wtot_feeid=a.ecoc_ecos_id where ecoc_state=1"+str+" order by coba_client";
		try {
			rs = db.GRS(sqlstr);
			while (rs.next()) {
	...
	public List<EmCommissionOutChangeModel> getwtout_aut_list(String str)
		...
		ResultSet rs = null;
		List<EmCommissionOutChangeModel> list = new ArrayList<EmCommissionOutChangeModel>();
		String sqlstr = "select coba_shortname,wtss_city,ecoc_addtype,emba_name,a.gid,a.cid,ecoc_id,ecoc_tapr_id,coba_client,ecoc_addtime,coab_shortname,case when g.ecoc_idcard<>'' then 1 else 0 end ts_st from EmCommissionOutChange a left join cobase b on b.cid=a.cid left join wtoutFee c on c.Wtot_feeid=a.ecoc_ecos_id left join embase d on d.gid=a.gid left join CoAgencyBaseCityRel e on e.cabc_id=c.coab_id left join CoAgencyBase f on f.coab_id=e.cabc_coab_id left join (select ecoc_idcard from EmCommissionOutChange where ecoc_state=1 group by ecoc_idcard having COUNT(ecoc_idcard)>1) g on g.ecoc_idcard=a.ecoc_idcard where ecoc_state=1 "+str+" order by ecoc_addtime";
		try {
			...
			while (rs.next()) {
				...
				model.setType(rs.getString("ts_st"));
				list.add(model);
			}
		} catch (Exception e) {
			System.out.print(e.toString());
	...
	public List<EmCommissionOutZYTModel> getci_excel(String id)
		...
		String sqlstr = "select a.gid t1,emba_name t2,isnull(emba_sex,0) t3,isnull(ecoc_mobile,0) t4,isnull(ecoc_phone,'') t5,a.cid t6,coba_company t7,ecoc_phone t8,ecoc_addtype t9,"
				...
				+ "'' T20,ecoc_salary T21,'' T22,i.st_num T23,e.city T24,'' T25,'' T26,'' T27,soin_title T28,'' T29,substring(replace(isnull(yl_date,''),'-','.'),1,7) T30,"
				...
				+ "'0' T106,'0' T107,'' T108,'0' T109,i.cz_num T110,'' T111,isnull(ecoc_remark,'') T112,'' T113,'' T114,'' T115,'' T116,'' T117,'' T118,'' T119,"
				...
				" left join EmCommissionOutCZ i on i.city=d.wtss_city"
	...
	// 获取委托出数据信息
		public List<EmCommissionOutZYTModel> getci_excel_f(String id)
				throws SQLException {
			dbconn db = new dbconn();
			ResultSet rs = null;
			List<EmCommissionOutZYTModel> list = new ArrayList<EmCommissionOutZYTModel>();
			String sqlstr = "select a.gid t1,emba_name t2,isnull(emba_sex,0) t3,isnull(ecoc_mobile,0) t4,isnull(ecoc_phone,'') t5,a.cid t6,coba_company t7,ecoc_phone t8,ecoc_addtype t9,"
					...
					+ "gs_em_total T62,case when wtss_shebaoco='受托方账户' then '管理费' else '公司自付' end T63,case when wtss_shebaoco='受托方账户' then '管理费' else '个人自付' end T64,'' T65,substring(replace(isnull(syu_date,''),'-','.'),1,7) T66,'0' T67,syu_co_base T68,syu_em_base T69,"
					+ "syu_co_total T70,syu_em_total T71,case when wtss_shebaoco='受托方账户' then '管理费' else '公司自付' end T72,case when wtss_shebaoco='受托方账户' then '管理费' else '个人自付' end T73,'' T74,substring(replace(isnull(house_date,''),'-','.'),1,7) T75,'0' T76,"
					+ "house_co_base T77,house_em_base T78,cast(cast(house_cp as money)*100 as varchar(10)) T79,cast(cast(house_op as money)*100 as varchar(10)) T80,house_co_total T81,house_em_total T82,case when wtss_gjjco='受托方账户' then '管理费' else '公司自付' end T83,case when wtss_gjjco='受托方账户' then '管理费' else '个人自付' end T84,'' T85,"
					+ "substring(replace(isnull(bc_date,''),'-','.'),1,7) T86,'0' T87,case when bc_co_total+bc_em_total=0 then '0' else bc_co_base end T88,case when bc_co_total+bc_em_total=0 then '0' else bc_em_base end T89,case when bc_co_total+bc_em_total=0 then '0' else bc_cp end T90,case when bc_co_total+bc_em_total=0 then '0' else bc_op end T91,"
					+ "bc_co_total T92,bc_em_total T93,case when wtss_gjjco='受托方账户' then '管理费' else '公司自付' end T94,case when wtss_gjjco='受托方账户' then '管理费' else '个人自付' end T95,substring(replace(isnull(fee_date,''),'-','.'),1,7) T96,'0' T97,'' T98,ecoc_welfare_sum T99,'' T100,"
					+ "'' T101,fee_total T102,substring(replace(isnull(file_date,''),'-','.'),1,7) T103,'0' T104,file_total T105,"
					+ "'0' T106,'0' T107,'' T108,'0' T109,i.cz_num T110,'' T111,isnull(ecoc_remark,'') T112,'' T113,'' T114,'' T115,'' T116,'' T117,'' T118,'' T119,"
	...
	public EmCommissionOutChangeModel getwt_state_list()
		...
		String sqlstr = "select (select COUNT(*) from EmCommissionOutChange where ecoc_state=1 and ecoc_addtype='新增') xa,(select COUNT(*) from EmCommissionOutChange where ecoc_state=1 and ecoc_addtype='离职') la,(select COUNT(*) from EmCommissionOutChange where ecoc_state=1 and ecoc_addtype='调整') ta,(select COUNT(*) from EmCommissionOutChange where ecoc_state=1 and ecoc_addtype='取消') qa";
		try {
			System.out.println(sqlstr);
			rs = db.GRS(sqlstr);
			while (rs.next()) {
