...
public class EmCommissionOut_AddReDal {
	...
	public List<EmCommissionOutStandardModel> getwtout_stand(String ecou_id)
		...
		String sqlstr = "select wtot_feeid,wtss_city+'-'+wtss_title+'-'+wtot_feetitle+'-'+coab_shortname title,coab_id from  View_wtservicestandard c left join EmCommissionOutChange a on c.cid=a.cid where ecoc_id="
				+ ecou_id + " and wtot_state=3 and wtot_ifview=1";
		try {
			System.out.println(sqlstr);
			rs = db.GRS(sqlstr);
			while (rs.next()) {
	...
	public List<EmCommissionOutFeeDetailChangeModel> getfeedetail(
		...
		String sqlstr = "select eofc_ecoc_id,eofc_name,eofc_em_base,eofc_co_base,isnull(eofc_op,'') eofc_op,isnull(eofc_cp,'') eofc_cp,eofc_month_sum,eofc_start_date,eofc_sicl_id,isnull(eofc_co_sum,0) eofc_co_sum,isnull(eofc_em_sum,0) eofc_em_sum,cast(year(eofc_start_date) as varchar(5))+'-'+cast(month(eofc_start_date) as varchar(5))+'-1' td1,cast(year(getdate()) as varchar(5))+'-'+cast(month(getdate()) as varchar(5))+'-1' td2 from EmCommissionOutFeeDetailChange where eofc_name in ('养老保险','医疗保险','大病医疗','生育保险','工伤保险','失业保险','住房公积金','补充公积金','服务费','档案费') and eofc_ecoc_id="
...
System.out.println(sqlstr);
			...
			while (rs.next()) {
				...
				model.setTempDate(rs.getDate("td2"));
				model.setTempDate1(rs.getDate("td2"));
