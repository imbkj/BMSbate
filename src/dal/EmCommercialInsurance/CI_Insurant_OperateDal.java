...
public class CI_Insurant_OperateDal {
	private dbconn conn = new dbconn();

	// 任务单编号更新
	public int upinsurantid(int ccsa_id, int tapr_id) throws Exception {
		int row = 0;
		String sqlstr = "update EmComInsuranceChange set ecic_tapr_id="
				+ tapr_id + " where ecic_id=" + ccsa_id + "";
	...
	public int Add_Insurant(String gid, String cid, String em1, String em2,
			...
			String em8, String em9, String em10, String em11,String getld,String compact_qd) {
		try {
	...
	// 商业保险信息变更
		public int Apply_Insurant(String gid, String cid,String getld,String username) {
			try {
				CallableStatement c = conn
	...
	public int ecin_del(String id) throws SQLException {
		...
		String sqlstr = "select ecic_inid,ecic_state from EmComInsuranceChange where ecic_id="
				+ id + "";
		try {
			rs = db.GRS(sqlstr);
			while (rs.next()) {
				String sqlstr1 = "delete EmComInsuranceChange where ecic_id="
						+ id + "";
				if(rs.getInt("ecic_state")==1){
				sqlstr2 = "delete EmComInsurance where ecin_id="
						+ rs.getString("ecic_inid") + "";
				}else{
					sqlstr2 = "update EmComInsurance set ecin_state=1,ecin_st_date=null where ecin_id="
							+ rs.getString("ecic_inid") + "";
				}
				rs1 = db.GRS(sqlstr1);
				rs2 = db.GRS(sqlstr2);
