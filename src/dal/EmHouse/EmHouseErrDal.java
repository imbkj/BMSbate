...
public class EmHouseErrDal {
	/**
	 * @Title: getListByOwnmonth
	 * @Description: TODO(检查当前月份是否有台后逻辑检查数据)
	 * @return
	 * @return List<EmHouseErrMonthModel> 返回类型
	 * @throws
	 */
	public List<EmHouseErrModel> getListByOwnmonth() {
		List<EmHouseErrModel> list = new ListModelList<>();
		dbconn db = new dbconn();
		String sql = "select * from EmHouseErr where ownmonth in (select top 1 ownmonth from emhouseupdate)";
		try {
			list = db.find(sql, EmHouseErrModel.class, null);
	...
	public List<EmHouseErrModel> getList(EmHouseErrModel em, boolean distinct,
			String columns) {
		List<EmHouseErrModel> list = new ListModelList<>();
		dbconn db = new dbconn();
		String sql = "select ";
	...
	public Integer checkerr() {
		...
		Integer i = 0;
		try {
			i = (Integer) db.callWithReturn("{?=call EmHouse_GJJErr_P_py(?)}",
					Types.INTEGER, UserInfo.getUsername());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public Integer Del() {
		dbconn db = new dbconn();
		Integer i = 0;
		String sql = "delete from emhouseerr where ownmonth in (select top 1 ownmonth from emhouseupdate)";
		try {
