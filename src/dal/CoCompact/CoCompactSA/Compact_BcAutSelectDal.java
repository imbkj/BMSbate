...
public class Compact_BcAutSelectDal {
	private static dbconn conn = new dbconn();

	// 获取所有公司合同数据集
	private ResultSet getCoCoBase() throws Exception {
		ResultSet rs = null;
		String sql = "SELECT * FROM CoCompactSA a left join CoCompact b on b.coco_id=a.ccsa_coco_id left join CoLatencyClient c on c.cola_id=b.coco_cola_id  where ccsa_state=0";
		rs = conn.GRS(sql);
		return rs;
	}
	...
	private ResultSet getCoCoBase(String str) throws Exception {
		...
		sql.append("SELECT * ");
		sql.append(" FROM CoCompactSA a left join CoCompact b on b.coco_id=a.ccsa_coco_id where ccsa_state=0 ");
		sql.append(str);
