...
public class Compact_BcUploadDal {
	...
	// 文件上传
	public int DocFileUpload(int cid, int gid, String doin_id, String url,
			String size, String addname,int coli_id,String co_type) {
		try {
			CallableStatement c = conn
					.getcall("CompactBcUpload_P_zzq(?,?,?,?,?,?,?,?,?)");
			
			System.out.println("-------------------");
			System.out.println(cid);
			System.out.println(gid);
			System.out.println(doin_id);
			System.out.println(url);
			System.out.println( size);
			System.out.println(addname);
			System.out.println(coli_id);
			System.out.println(co_type);
			
			c.setInt(1, cid);
			c.setInt(2, gid);
			c.setString(3, doin_id);
			c.setString(4, url);
			c.setString(5, size);
			c.setString(6, addname);
			c.setInt(7, coli_id);
			c.setString(8, co_type);
	...
	// 公司合同上传
	public int UDocFileUpload(int cid, int gid, String doin_id, String url,
			String size, String addname) {

		try {
			CallableStatement c = conn
					.getcall("CompactUpAut_P_zzq(?,?,?,?,?,?,?)");
	...
	// 公司合同上传审核
	public int AuDocFileUpload(int cid, int gid, int doin_id) {

		try {
			CallableStatement c = conn
					.getcall("CompactUpload_P_zzq(?,?)");
			c.setInt(1, doin_id);
	...
	// 获取该公司编号
		public List<CI_Insurant_ListModel> getcid(int coco_id) throws SQLException {
			dbconn db = new dbconn();
			ResultSet rs = null;
			List<CI_Insurant_ListModel> list = new ArrayList<CI_Insurant_ListModel>();
			String sqlstr = "select a.cid,coba_company from CoCompact a left join cobase b on b.cid=a.cid where coco_id="
					+ coco_id;
			try {
				rs = db.GRS(sqlstr);
				while (rs.next()) {
		...
		// 获取该潜在客户编号
				public List<CI_Insurant_ListModel> getcola_id(int coco_id) throws SQLException {
					dbconn db = new dbconn();
					ResultSet rs = null;
					List<CI_Insurant_ListModel> list = new ArrayList<CI_Insurant_ListModel>();
					String sqlstr = "select coco_cola_id from cocompact where coco_id="
							+ coco_id;
					try {
						rs = db.GRS(sqlstr);
						while (rs.next()) {
							CI_Insurant_ListModel model = new CI_Insurant_ListModel();
							model.setCid(rs.getString("coco_cola_id"));
							list.add(model);
						}
					} catch (Exception e) {
						System.out.print(e.toString());
