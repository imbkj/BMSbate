...
public class Compact_PrintSelectDal {
	private static dbconn conn = new dbconn();

	// 获取所有公司合同数据集
	private ResultSet getCoCoBase() throws Exception {
		ResultSet rs = null;
		String sql = "SELECT * FROM CoCompact_CoLa_CoBase_V where coco_state=2 order by coco_id desc";
		rs = conn.GRS(sql);
		return rs;
	}
	...
	private ResultSet getCoCoBase(String str) throws Exception {
		...
		sql.append("SELECT * ");
		sql.append(" FROM CoCompact_CoLa_CoBase_V where coco_state=2 ");
		sql.append(str);
